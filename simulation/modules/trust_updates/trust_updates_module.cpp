...
void TrustUpdatesModule::execute_province(uint32_t province_idx,
    ...
    for (uint32_t npc_id : npc_ids) {
        ...
        for (const auto& rel : rels) {
            ...
            for (const auto& memory : npc->memory_log) {
                if (memory.tick_timestamp != state.current_tick) continue;
                if (memory.subject_id != rel.target_npc_id) continue;
                total_delta += memory.emotional_weight * 0.1f;
            }

            if (std::abs(total_delta) < 1e-6f) continue;
            ...
            if (total_delta < 0.0f && is_catastrophic_loss(total_delta, resulting)) {
                ...
                updated_rel.recovery_ceiling = update_recovery_ceiling(
                    rel.recovery_ceiling, ceiling);
            }
            ...
            if (is_significant_change(total_delta)) {
                ...
                npc_delta.new_memory_entry = MemoryEntry{
                    state.current_tick, MemoryType::interaction,
                    rel.target_npc_id, weight, 0.01f, false
                };
            }

...
float ProtectionRacketsModule::compute_demand_per_tick(
...
float ProtectionRacketsModule::compute_grievance_contribution(
    float demand_per_tick,
    float grievance_per_demand_unit)
{
    return demand_per_tick * grievance_per_demand_unit;
}

float ProtectionRacketsModule::compute_refusal_probability(
    bool is_defensive_incumbent,
...
RacketEscalationStage ProtectionRacketsModule::determine_escalation_stage(uint32_t ticks_overdue) {
    if (ticks_overdue >= ABANDONMENT_THRESHOLD) return RacketEscalationStage::abandonment;
    if (ticks_overdue >= VIOLENCE_THRESHOLD)     return RacketEscalationStage::violence;
    if (ticks_overdue >= PROPERTY_DAMAGE_THRESHOLD) return RacketEscalationStage::property_damage;
    if (ticks_overdue >= WARNING_THRESHOLD)      return RacketEscalationStage::warning;
...
float ProtectionRacketsModule::compute_violence_le_multiplier(
    float base_fill_rate,
...
void ProtectionRacketsModule::execute_province(
    ...
    DeltaBuffer& province_delta)
{
    ...
    std::sort(rackets_.begin(), rackets_.end(),
              [](const ProtectionRacket& a, const ProtectionRacket& b) {
                  return a.id < b.id;
              });
    ...
    for (auto& racket : rackets_) {
        ...
        if (!target_biz || target_biz->province_id != province.id) continue;

        // Skip lapsed and expelled rackets
        if (racket.status == RacketStatus::lapsed ||
            racket.status == RacketStatus::expelled) {
            continue;
        }

        // Update demand per tick based on current business revenue
        racket.demand_per_tick = compute_demand_per_tick(
            target_biz->revenue_per_tick, DEMAND_RATE);
        ...
        racket.community_grievance_contribution = compute_grievance_contribution(
            racket.demand_per_tick, GRIEVANCE_PER_DEMAND_UNIT);
            ...
            if (new_stage != racket.escalation_stage) {
                ...
                switch (new_stage) {
                    case RacketEscalationStage::warning: {
                        ...
                        owner_delta.new_memory_entry = MemoryEntry{
                            state.current_tick,
                    ...
                    case RacketEscalationStage::property_damage: {
                        ...
                        ev.new_token = EvidenceToken{
                            ...
                            PROPERTY_DAMAGE_SEVERITY, 0.002f,

...
class RandomEventsModule : public ITickModule {
   ...
   private:
    ...
    void process_active_events(const WorldState& state, const Province& province,
                               DeltaBuffer& province_delta);
    void apply_per_tick_effects(const WorldState& state, const Province& province,
                                ActiveRandomEvent& event, DeltaBuffer& province_delta);
    void apply_natural_per_tick(const WorldState& state, const Province& province,
                                const ActiveRandomEvent& event, DeltaBuffer& province_delta);
    void apply_accident_per_tick(const WorldState& state, const Province& province,
                                 ActiveRandomEvent& event, DeltaBuffer& province_delta);
    void apply_economic_per_tick(const WorldState& state, const Province& province,
                                 const ActiveRandomEvent& event, DeltaBuffer& province_delta);
    void apply_human_per_tick(const WorldState& state, const Province& province,
                              const ActiveRandomEvent& event, DeltaBuffer& province_delta);
    float compute_economic_volatility(const WorldState& state, const Province& province) const;
    EventCategory select_category(float climate_stress, float instability, float infra_rating,
                                  float economic_volatility, DeterministicRNG& rng) const;
    const RandomEventTemplate* select_template(EventCategory category, const Province& province,
                                               DeterministicRNG& rng) const;
    void roll_for_new_event(const WorldState& state, const Province& province,
                            DeterministicRNG& rng, DeltaBuffer& province_delta);
    void apply_immediate_effects(const WorldState& state, const Province& province,
                                 ActiveRandomEvent& event, DeltaBuffer& province_delta);
};

...
struct TradeInfrastructureConstants {
    ...
    static constexpr float speed_for_mode(TransportMode mode) {
        switch (mode) {
            case TransportMode::road:  return mode_speed_road;
            case TransportMode::rail:  return mode_speed_rail;
            case TransportMode::sea:   return mode_speed_sea;
            case TransportMode::river: return mode_speed_river;
            case TransportMode::air:   return mode_speed_air;
            default:                   return mode_speed_road;
        }
    }
};
...
public:
    ...
    static uint32_t calculate_transit_ticks(const RouteProfile& route,
                                            TransportMode mode);
    ...
    static bool apply_perishable_decay(TransitShipment& shipment,
                                       float decay_rate);

    // --- Static utility: interception check ---
    // Rolls against the shipment's interception risk.
    // Returns true if intercepted.
    static bool check_interception(const TransitShipment& shipment,
...
private:
    ...
    void process_interception_checks(uint32_t current_tick,
                                     DeltaBuffer& delta,
                                     DeterministicRNG& rng);

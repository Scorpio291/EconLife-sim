...
public:
    ...
    static bool is_mechanism_valid_for_scale(CompensationMechanism mechanism,
                                             BusinessScale scale);

    // Compute quarterly net profit.
    static float compute_quarterly_net_profit(float revenue_per_tick,
                                              float cost_per_tick,
    ...
    static bool is_board_approved(const BoardComposition& board,
...
private:
    ...
    void process_salary_payments(const NPCBusiness& business,
                                 BusinessCompensationRecord& record,
                                 const WorldState& state,
                                 DeltaBuffer& delta);

    // Step 2: Process owner's draw (micro businesses only).
    void process_owners_draw(const NPCBusiness& business,
                             BusinessCompensationRecord& record,
                             const WorldState& state,
                             DeltaBuffer& delta);

    // Step 3: Process quarterly bonus distribution.
    void process_quarterly_bonus(const NPCBusiness& business,
                                 BusinessCompensationRecord& record,
                                 const WorldState& state,
                                 DeltaBuffer& delta);

    // Step 4: Process quarterly dividend payout.
    void process_quarterly_dividend(const NPCBusiness& business,
                                    BusinessCompensationRecord& record,
                                    const WorldState& state,
                                    DeltaBuffer& delta);

    // Step 5: Process equity grant vesting (full_package only, large businesses).
    void process_equity_vesting(const NPCBusiness& business,
                                BusinessCompensationRecord& record,
                                const WorldState& state,
                                DeltaBuffer& delta);

    // Step 6: Update retained earnings from this tick's revenue and costs.
    void update_retained_earnings(const NPCBusiness& business,
                                  BusinessCompensationRecord& record);

    // --- Lookup helpers ---

    static const NPCBusiness* find_business(const WorldState& state,

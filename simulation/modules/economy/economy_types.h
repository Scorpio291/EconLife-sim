...
struct ExecutiveCompensation {
    ...
    float
        dividend_yield_target;  // fraction of retained earnings distributed per quarter
...
struct NPCBusiness {
    ...
    uint32_t strategic_decision_tick;  // when they next make a quarterly decision; see
                                       // dispatch_day_offset
    uint8_t dispatch_day_offset;       // 0-29; set at world gen as hash(id) % 30.
                                       // First decision at world_start_tick + offset;
                                       // then every 30 ticks. Spreads NPC business
                                       // dispatch load evenly across each month.
                                       // Player-owned businesses dispatch on command.
    ActorTechnologyState
        actor_tech_state;  // per-actor technology portfolio (see R&D doc Part 3.5)
                           ...
                           //   facility.tech_tier weighted by
                           //   actor_tech_state.maturation_of(recipe.key_technology_node)
    ...
    float deferred_salary_liability;  // Accumulated unpaid salary when business.cash <
                                      // salary_per_tick. Deferred salary is paid first when cash
                                      // recovers. Sustained deferred salary (>
                                      // config.business.deferred_salary_max_ticks ticks) generates
                                      // a witnessed_wage_theft memory entry.
    ...
    float accounts_payable_float;  // estimated outstanding supplier obligations.
                                   // Approximated as: cost_per_tick x
                                   // config.business.payment_lag_ticks. Conceptually: the business
                                   // owes its last N ticks of input costs. Used by: bankruptcy
                                   // evaluation (if cash < accounts_payable_float,
                                   //   business is cash-flow insolvent even if technically
                                   //   profitable).

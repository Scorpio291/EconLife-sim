...
class MoneyLaunderingModule : public ITickModule {
   public:
    ...
    static float compute_transfer_this_tick(float launder_rate_per_tick, float dirty_amount,
                                            float laundered_so_far);

    // Compute clean amount after conversion loss
    static float compute_clean_amount(float transfer_this_tick, float conversion_loss_rate);

    // Check if structuring evidence should be generated this tick
    static bool should_generate_structuring_evidence(uint32_t current_tick, uint32_t started_tick,
                                                     uint32_t structuring_token_interval);

    // Check if shell chain evidence should be generated for a chain node
    static bool should_generate_shell_chain_evidence(uint32_t current_tick, uint32_t started_tick,
                                                     uint32_t shell_chain_evidence_interval);

    // Compute crypto evidence probability per tick
    static float compute_crypto_evidence_probability(float launder_rate_per_tick,
                                                     float mixer_traceability, float max_le_skill,
                                                     float crypto_evidence_skill_divisor);

    // Compute commingling capacity cap
    static float compute_commingling_capacity(float business_revenue_per_tick,
                                              float commingle_capacity_fraction,
                                              float rate_commingle_max);

    // Compute FIU structuring suspicion score
    static float compute_fiu_structuring_suspicion(uint32_t sub_threshold_deposit_count,

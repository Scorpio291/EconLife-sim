...
float MediaSystemModule::compute_evidence_weight(
    const std::vector<float>& actionabilities)
{
    if (actionabilities.empty()) return 0.0f;
    float sum = 0.0f;
    for (float a : actionabilities) sum += a;
    return std::clamp(sum / static_cast<float>(actionabilities.size()), 0.0f, 1.0f);
}

bool MediaSystemModule::evaluate_editorial_filter(
    float editorial_independence, float owner_suppression_rate,
    float roll)
{
...
float MediaSystemModule::compute_pickup_probability(
    float evidence_weight, float other_outlet_credibility,
...
float MediaSystemModule::compute_social_amplification(
    float current_amplification, float social_reach,
    float social_multiplier, float evidence_weight)
{
    float amp = current_amplification * social_reach * social_multiplier *
                (1.0f + evidence_weight);
    if (std::isnan(amp)) return 0.0f;
    return std::max(0.0f, amp);
}

float MediaSystemModule::compute_exposure_delta(
    float amplification, float exposure_per_unit)
{
    return std::max(0.0f, amplification * exposure_per_unit);
}

bool MediaSystemModule::is_within_propagation_window(
    uint32_t published_tick, uint32_t current_tick,
    uint32_t window_ticks)
{
    if (current_tick < published_tick) return false;
    ...
    for (const auto& outlet : outlets_) {
        ...
        for (uint32_t journalist_id : sorted_journalist_ids) {
            ...
            for (const auto& knowledge : journalist->known_evidence) {
                if (knowledge.type != KnowledgeType::evidence_token) continue;
                ...
                if (!token) continue;
                if (token->actionability < Constants::crisis_evidence_threshold) continue;
                ...
                if (is_player_owned && story_about_player) {
                    ...
                    if (!evaluate_editorial_filter(
                            outlet.editorial_independence,
                            Constants::owner_suppression_base_rate,
                            roll))
                    {
                ...
                story.tone = (token->target_npc_id == state.player->id)
                                 ? StoryTone::damaging : StoryTone::neutral;
...
void MediaSystemModule::propagate_stories(const WorldState& state,
                                            DeltaBuffer& delta)
{
    ...
    for (auto& story : active_stories_) {
        if (!story.is_active) continue;
        if (!is_within_propagation_window(story.published_tick,
                                            state.current_tick,
        ...
        // Cross-outlet pickup
        for (const auto& outlet : outlets_) {
            if (outlet.id == story.outlet_id) continue;
            ...
            if (outlet.province_id != story_province) continue;

            float pickup_prob = compute_pickup_probability(
                story.evidence_weight,
                outlet.credibility,
                Constants::cross_outlet_pickup_rate);

            // Deterministic pickup: use (story.id * outlet.id) % 100 as roll
            float roll = static_cast<float>((story.id * outlet.id) % 100) / 100.0f;
            if (roll < pickup_prob) {
                story.amplification += outlet.reach *
                                        Constants::cross_outlet_amplification_factor;
            }
        }

        // Social media amplification
        for (const auto& outlet : outlets_) {
            if (outlet.type != MediaOutletType::social_platform) continue;
            ...
            float social_amp = compute_social_amplification(
                story.amplification,
                outlet.reach,
                Constants::social_amplification_multiplier,
                story.evidence_weight);
...
void MediaSystemModule::convert_exposure(const WorldState& state,
                                           DeltaBuffer& delta)
{
...
void MediaSystemModule::expire_old_stories(uint32_t current_tick) {
    for (auto& story : active_stories_) {
        if (story.is_active &&
            !is_within_propagation_window(story.published_tick, current_tick,
                                            Constants::propagation_window_ticks))
        {

...
struct ProvinceSkillKey {
    ...
    bool operator==(const ProvinceSkillKey& o) const noexcept {
        return province_id == o.province_id && domain == o.domain;
    }
};
...
struct ProvinceSkillKeyHash {
    std::size_t operator()(const ProvinceSkillKey& k) const noexcept {
        ...
        return std::hash<uint64_t>{}((static_cast<uint64_t>(k.province_id) << 8) |
                                     static_cast<uint64_t>(k.domain));
    }
};

using RegionalWageMap = std::unordered_map<ProvinceSkillKey, float, ProvinceSkillKeyHash>;

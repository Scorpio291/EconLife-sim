...
void write_province(ByteWriter& w, const Province& p) {
    ...
    if (has_cohort) {
        ...
        // v28: asabiya — what a people can do together, forged at frontiers and decaying
...
Province read_province(ByteReader& r, uint32_t schema_ver) {
    ...
    if (has_cohort) {
        ...
        // v28: asabiya. Older saves default to the seed (the struct default), which is
...
RestoreResult PersistenceModule::deserialize_body(const std::vector<uint8_t>& data,
    ...
    // pending_racket_seeds follows the same same-tick contract
    // (criminal_operations Tier 7 emits, protection_rackets Tier 8 drains).
    // Defensively reset on load.
    ...
    // pending_informant_countermeasures: player_actions Tier 0 emits,
    // informant_system Tier 9 drains the same tick. Defensively reset on load.
    out_state.pending_informant_countermeasures.clear();

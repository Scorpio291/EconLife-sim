...
void RegionalConditionsModule::execute_province(uint32_t province_idx,
    ...
    {
        float raw_inequality = demographics.income_high_fraction
        ...
        rdelta.inequality_delta = INEQUALITY_CONVERGENCE_RATE
                                  * (target_inequality - conditions.inequality_index);
    }
    ...
    {
        float target_crime = conditions.criminal_dominance_index
                             * (1.0f - conditions.stability_score);
        constexpr float CRIME_CONVERGENCE_RATE = 0.002f;
        rdelta.crime_rate_delta = CRIME_CONVERGENCE_RATE
                                  * (target_crime - conditions.crime_rate);
    }
    ...
    // Recalculate criminal dominance from current conditions.
    {
        ...
        rdelta.criminal_dominance_delta = DOMINANCE_CONVERGENCE_RATE
                                          * (new_dominance - conditions.criminal_dominance_index);
    }
    ...
    // Community response stage amplifies grievance above stage 2.
    {
        ...
        float stage_contribution = (community.response_stage > 2)
                                   ? static_cast<float>(community.response_stage - 2)
                                     * GRIEVANCE_STAGE_WEIGHT
                                   : 0.0f;

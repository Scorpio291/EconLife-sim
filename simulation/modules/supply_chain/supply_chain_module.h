...
class SupplyChainModule : public ITickModule {
   public:
    ...
    static uint32_t compute_transit_ticks(const RouteProfile& route, float mode_speed,
   ...
   private:
    ...
    void collect_buy_orders(uint32_t province_id, const WorldState& state,
    ...
    void match_local(uint32_t province_id, const WorldState& state, DeltaBuffer& delta,
                     std::vector<SellOffer>& offers, std::vector<BuyOrder>& orders) const;

    // Dispatch inter-province shipments for unfulfilled buy orders.
    void dispatch_inter_province(uint32_t province_id, const WorldState& state, DeltaBuffer& delta,
                                 const std::vector<BuyOrder>& unfulfilled_orders,
                                 DeterministicRNG& rng) const;

    // Process transit arrivals from deferred work queue.
    void process_transit_arrivals(const WorldState& state, DeltaBuffer& delta) const;

    // Process criminal shipment interception checks.
    void process_interception_checks(const WorldState& state, DeltaBuffer& delta,
                                     DeterministicRNG& rng) const;

    // Process LOD 1 import offers.
    void process_lod1_imports(const WorldState& state, DeltaBuffer& delta) const;
};

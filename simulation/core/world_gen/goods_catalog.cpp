...
bool GoodsCatalog::load_csv(const std::string& filepath) {
    ...
    while (std::getline(file, line)) {
        ...
        // Parse tier (uint8_t)
        try {
            good.tier = static_cast<uint8_t>(std::stoi(fields[2]));
        } catch (...) {
            continue;
        }
        ...
        try {
            good.base_price = std::stof(fields[5]);
        } catch (...) {
            continue;
        }
        ...
        try {
            good.era_available = static_cast<uint8_t>(std::stoi(fields[8]));
        } catch (...) {
            good.era_available = 1;
        }
...
std::vector<const GoodDefinition*> GoodsCatalog::goods_available_at(uint8_t era,
                                                                    uint8_t max_tier) const {

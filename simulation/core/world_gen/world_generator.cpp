...
static const char* province_name_suffixes[] = {
    "Haven", "Ridge", "Valley", "Shore", "Fields", "Gate",
    "Brook", "Cliff", "Marsh", "Dale", "Ford", "Heights",
    "Landing", "Point", "Mill", "Creek", "Bridge", "Hollow",
};
...
WorldGenerator::ProvinceArchetype WorldGenerator::assign_archetype(
        DeterministicRNG& rng, uint32_t province_idx, uint32_t province_count) {
    ...
    if (province_idx < 6) {
        ...
        static constexpr ProvinceArchetype base_order[] = {
            ProvinceArchetype::industrial_hub,
            ProvinceArchetype::agricultural,
            ProvinceArchetype::resource_rich,
            ProvinceArchetype::coastal_trade,
            ProvinceArchetype::financial_center,
            ProvinceArchetype::mixed_economy,
        };
...
void WorldGenerator::apply_archetype(Province& province, ProvinceArchetype archetype,
    ...
    default:
        ...
        province.geography.is_landlocked = (rng.next_uint(3) == 0);
        province.geography.port_capacity = province.geography.is_landlocked ? 0.0f : 0.3f + rng.next_float() * 0.3f;
        province.geography.river_access = 0.3f + rng.next_float() * 0.4f;
    ...
    province.climate.temperature_avg_c = 25.0f - province.geography.latitude * 0.3f +
                                          rng.next_float() * 5.0f;
    province.climate.temperature_min_c = province.climate.temperature_avg_c - 15.0f -
                                          rng.next_float() * 10.0f;
    province.climate.temperature_max_c = province.climate.temperature_avg_c + 10.0f +
                                          rng.next_float() * 10.0f;
    ...
    province.political.corruption_index = config.corruption_baseline +
                                           rng.next_float() * 0.15f;
    ...
    province.conditions.criminal_dominance_index = config.criminal_baseline +
                                                    rng.next_float() * 0.05f;
    ...
    province.community.grievance_level = std::max(province.community.grievance_level,
                                                   trauma_grievance_floor);
    province.community.institutional_trust = std::min(province.community.institutional_trust,
                                                       trauma_trust_ceiling);
...
void WorldGenerator::create_provinces(WorldState& world, DeterministicRNG& rng,
    ...
    for (uint32_t p = 0; p < config.province_count; ++p) {
        ...
        uint32_t suffix_idx = rng.next_uint(
            static_cast<uint32_t>(sizeof(province_name_suffixes) / sizeof(province_name_suffixes[0])));
        province.fictional_name = std::string(archetype_name_prefixes[static_cast<uint8_t>(archetype)]) +
...
void WorldGenerator::create_province_links(WorldState& world) {
    ...
    for (uint32_t p = 0; p < n; ++p) {
        auto add_link = [&](uint32_t neighbor_idx, LinkType type) {
            ...
            link.type = type;
            link.shared_border_km = (type == LinkType::Maritime) ? 0.0f : 40.0f + static_cast<float>(p) * 5.0f;
            link.transit_terrain_cost = 0.2f + world.provinces[p].geography.terrain_roughness * 0.3f;
            link.infrastructure_bonus = std::min(world.provinces[p].infrastructure_rating,
...
void WorldGenerator::create_markets(WorldState& world, DeterministicRNG& rng,
    ...
    if (available_goods.empty()) {
        ...
        static const struct { const char* name; float price; } fallback_goods[] = {
            {"food", 25.0f}, {"raw_materials", 15.0f}, {"consumer_goods", 50.0f},
            {"industrial_goods", 80.0f}, {"energy", 40.0f}, {"services", 60.0f},
    ...
    for (uint32_t p = 0; p < static_cast<uint32_t>(world.provinces.size()); ++p) {
        ...
        for (const auto* good : available_goods) {
            ...
            } else if (good->category == "metals" &&
                       province.infrastructure_rating > 0.6f) {
                supply_base *= 1.2f;
            }
...
void WorldGenerator::create_npcs(WorldState& world, DeterministicRNG& rng,
    ...
    static constexpr uint32_t total_weight = 40+5+8+3+3+3+2+3+3+5+2+1+1+4+3+2+1+8+1+1;
    ...
    for (uint32_t p = 0; p < static_cast<uint32_t>(world.provinces.size()); ++p) {
        ...
        uint32_t npcs_for_province = std::max(1u,
            static_cast<uint32_t>(config.npc_count * pop_fraction + 0.5f));
        ...
        for (uint32_t i = 0; i < npcs_for_province; ++i) {
            ...
            weight_sum = 0.0f;
            for (int w = 0; w < 8; ++w) weight_sum += weights[w];
            for (int w = 0; w < 8; ++w) weights[w] /= weight_sum;
...
void WorldGenerator::create_businesses(WorldState& world, DeterministicRNG& rng,
    ...
    uint32_t total_businesses = std::max(
        static_cast<uint32_t>(world.provinces.size()),
    ...
    for (const auto& npc : world.significant_npcs) {
        if (npc.role == NPCRole::corporate_executive ||
            npc.role == NPCRole::middle_manager ||
            npc.role == NPCRole::criminal_operator ||
            npc.role == NPCRole::fixer) {
    ...
    static constexpr uint32_t formal_sector_count = sizeof(formal_sectors) / sizeof(formal_sectors[0]);
    ...
    for (uint32_t b = 0; b < total_businesses; ++b) {
        ...
        if (owner.province_id < world.provinces.size()) {
            province_wealth = world.provinces[owner.province_id].demographics.income_high_fraction * 2.0f +
        ...
        biz.cost_per_tick = biz.revenue_per_tick * (0.6f + rng.next_float() * 0.3f); // 60-90% cost ratio
        ...
        biz.default_activity_scope = is_criminal

...
struct EraDefinition {
    ...
    // having the smelters, the ore trade and the smiths is. Knowledge can spike —
    // information is cheap to copy — but capital has to be accumulated out of real
    // surplus at a physical rate and wears out, which is what keeps advancement
    // honest. 0 = not capital-gated.
    float capital_to_advance = 0.0f;
};

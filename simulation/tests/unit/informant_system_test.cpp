...
TEST_CASE("Informant: risk factor computation", "[informant_system][tier9]") {
    ...
    REQUIRE_THAT(InformantSystemModule::compute_risk_factor(0.2f), WithinAbs(0.24f, 0.01f));
    // High risk tolerance (0.8) -> low risk factor: (1-0.8)*0.30 = 0.06
    REQUIRE_THAT(InformantSystemModule::compute_risk_factor(0.8f), WithinAbs(0.06f, 0.01f));
}

TEST_CASE("Informant: trust factor computation", "[informant_system][tier9]") {
    // Low trust (0.2) -> high factor: (1-0.2)*0.25 = 0.20
    REQUIRE_THAT(InformantSystemModule::compute_trust_factor(0.2f), WithinAbs(0.20f, 0.01f));
    // High trust (0.8) -> low factor: (1-0.8)*0.25 = 0.05
    REQUIRE_THAT(InformantSystemModule::compute_trust_factor(0.8f), WithinAbs(0.05f, 0.01f));
}
...
TEST_CASE("Informant: flip probability full formula", "[informant_system][tier9]") {
    ...
    // clamped to MAX 0.20
    float prob = InformantSystemModule::compute_flip_probability(0.10f, 0.5f, 0.5f, 0, 0);
    REQUIRE_THAT(prob, WithinAbs(0.20f, 0.01f));
}

TEST_CASE("Informant: flip probability capped at MAX", "[informant_system][tier9]") {
    float prob = InformantSystemModule::compute_flip_probability(0.10f, 0.0f, 0.0f, 0, 0);
    // 0.10 + 0.30 + 0.25 = 0.65, capped to 0.20
    REQUIRE_THAT(prob, WithinAbs(0.20f, 0.01f));
}

TEST_CASE("Informant: mutual incrimination reduces flip", "[informant_system][tier9]") {
    float without = InformantSystemModule::compute_flip_probability(0.10f, 0.5f, 0.5f, 0, 0);
    float with_mutual = InformantSystemModule::compute_flip_probability(0.10f, 0.5f, 0.5f, 2, 0);
...
TEST_CASE("Informant: compartmentalization reduces flip", "[informant_system][tier9]") {
    ...
    float without = InformantSystemModule::compute_flip_probability(0.05f, 0.9f, 0.9f, 0, 0);
    float with_compart = InformantSystemModule::compute_flip_probability(0.05f, 0.9f, 0.9f, 0, 3);
...
TEST_CASE("Informant: high trust suppresses flip", "[informant_system][tier9]") {
    ...
    float low_trust = InformantSystemModule::compute_flip_probability(0.02f, 0.9f, 0.5f, 0, 0);
    float high_trust = InformantSystemModule::compute_flip_probability(0.02f, 0.9f, 0.9f, 0, 0);
    REQUIRE(high_trust < low_trust);
}

TEST_CASE("Informant: flip probability non-negative", "[informant_system][tier9]") {
    // Extreme suppression
    float prob = InformantSystemModule::compute_flip_probability(0.10f, 1.0f, 1.0f, 5, 5);
    REQUIRE(prob >= 0.0f);
}

TEST_CASE("Informant: constants match spec", "[informant_system][tier9]") {
    REQUIRE_THAT(InformantSystemModule::MAX_FLIP_PROBABILITY, WithinAbs(0.20f, 0.001f));
    REQUIRE_THAT(InformantSystemModule::PAY_SILENCE_COST, WithinAbs(50000.0f, 1.0f));
    REQUIRE_THAT(InformantSystemModule::VIOLENCE_LE_MULTIPLIER, WithinAbs(3.0f, 0.01f));
}

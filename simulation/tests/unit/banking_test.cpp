...
TEST_CASE("test_interest_rate_reflects_credit_risk", "[banking][tier5]") {
    ...
    float rate_high = BankingModule::compute_interest_rate(0.9f, false);

    // Low credit score (0.3): higher rate.
    float rate_low = BankingModule::compute_interest_rate(0.3f, false);
...
TEST_CASE("test_collateral_reduces_interest_rate", "[banking][tier5]") {
    float rate_unsecured = BankingModule::compute_interest_rate(0.6f, false);
    float rate_secured = BankingModule::compute_interest_rate(0.6f, true);

    REQUIRE(rate_secured < rate_unsecured);
    REQUIRE_THAT(rate_unsecured - rate_secured,
                 WithinAbs(BankingModule::Constants::collateral_rate_discount, 0.000001f));
}
...
TEST_CASE("test_criminal_informal_default_no_credit_impact", "[banking][tier5]") {
    ...
    DeltaBuffer delta{};
    for (uint32_t i = 0; i <= BankingModule::Constants::default_grace_ticks; ++i) {
        delta = DeltaBuffer{};
        module.execute(state, delta);
    }
...
TEST_CASE("test_compute_interest_rate_high_credit", "[banking][tier5]") {
    ...
    float rate = BankingModule::compute_interest_rate(1.0f, false);
...
TEST_CASE("test_compute_interest_rate_low_credit", "[banking][tier5]") {
    ...
    float rate = BankingModule::compute_interest_rate(0.0f, false);
...
TEST_CASE("test_compute_max_loan_amount", "[banking][tier5]") {
    ...
    float max_loan = BankingModule::compute_max_loan_amount(100.0f);
    REQUIRE_THAT(max_loan, WithinAbs(109500.0f, 1.0f));
}

TEST_CASE("test_compute_max_loan_amount_zero_revenue", "[banking][tier5]") {
    float max_loan = BankingModule::compute_max_loan_amount(0.0f);
...
TEST_CASE("test_interest_rate_with_collateral_at_max_credit", "[banking][tier5]") {
    ...
    float rate = BankingModule::compute_interest_rate(1.0f, true);
...
TEST_CASE("test_interest_rate_never_negative", "[banking][tier5]") {
    ...
    rate = BankingModule::compute_interest_rate(1.0f, false);
    REQUIRE(rate >= 0.0f);

    // And at zero credit.
    rate = BankingModule::compute_interest_rate(0.0f, true);
    REQUIRE(rate >= 0.0f);
}

...
TEST_CASE("MoneyLaundering: compute_transfer_this_tick basic",
...
TEST_CASE("MoneyLaundering: transfer zero when fully laundered",
          "[money_laundering][tier8]") {
...
TEST_CASE("MoneyLaundering: conversion_loss_deducted",
          "[money_laundering][tier8]") {
...
TEST_CASE("MoneyLaundering: structuring evidence at interval",
          "[money_laundering][tier8]") {
    // Every 7 ticks
...
TEST_CASE("MoneyLaundering: shell chain evidence at interval",
          "[money_laundering][tier8]") {
...
TEST_CASE("MoneyLaundering: crypto evidence scales with investigator skill",
          "[money_laundering][tier8]") {
    // Low skill
    float prob_low = MoneyLaunderingModule::compute_crypto_evidence_probability(
        500.0f, 0.5f, 2.0f, 10.0f);
    // High skill
    float prob_high = MoneyLaunderingModule::compute_crypto_evidence_probability(
        500.0f, 0.5f, 8.0f, 10.0f);
    // High skill should be 4x the low skill probability
    REQUIRE_THAT(prob_high, WithinAbs(prob_low * 4.0f, 0.01f));
}

TEST_CASE("MoneyLaundering: commingling capped by business revenue",
          "[money_laundering][tier8]") {
...
TEST_CASE("MoneyLaundering: commingling capped by absolute max",
          "[money_laundering][tier8]") {
...
TEST_CASE("MoneyLaundering: FIU detects structuring pattern",
          "[money_laundering][tier8]") {
...
TEST_CASE("MoneyLaundering: FIU no detection below threshold",
          "[money_laundering][tier8]") {
...
TEST_CASE("MoneyLaundering: structuring transfers and generates evidence",
    ...
    for (uint32_t t = 1; t <= 7; ++t) {
        float transfer = MoneyLaunderingModule::compute_transfer_this_tick(500.0f, 10000.0f, laundered);
        laundered += transfer;
...
TEST_CASE("MoneyLaundering: NPC org capacity limit",
          "[money_laundering][tier8]") {

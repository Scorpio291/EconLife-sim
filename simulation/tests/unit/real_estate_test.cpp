...
PropertyListing make_test_property(uint32_t id,
                                    PropertyType type,
...
TEST_CASE("test_asking_price_converges_upward", "[real_estate][tier4]") {
    PropertyListing prop = make_test_property(
        1, PropertyType::residential, 0, 100, 150000.0f, 100000.0f);

    // asking = 100,000, market = 150,000
    // gap = 150,000 - 100,000 = 50,000
    // convergence: 100,000 + 50,000 * 0.05 = 102,500
    RealEstateModule::converge_asking_price(
        prop, RealEstateConstants::price_convergence_rate);
...
TEST_CASE("test_asking_price_converges_downward", "[real_estate][tier4]") {
    PropertyListing prop = make_test_property(
        1, PropertyType::residential, 0, 100, 80000.0f, 100000.0f);
...
TEST_CASE("test_asking_price_at_market_value_unchanged", "[real_estate][tier4]") {
    PropertyListing prop = make_test_property(
        1, PropertyType::residential, 0, 100, 100000.0f, 100000.0f);
...
TEST_CASE("test_asking_price_never_negative", "[real_estate][tier4]") {
    PropertyListing prop = make_test_property(
        1, PropertyType::residential, 0, 100, 0.0f, 1.0f);
...
TEST_CASE("test_property_transaction_transfers_ownership", "[real_estate][tier4]") {
    ...
    RealEstateModule module;
    auto prop = make_test_property(
        1, PropertyType::residential, 0, 100, 200000.0f);
    module.add_property(prop);
...
TEST_CASE("test_industrial_yield_higher_than_commercial_higher_than_residential", "[real_estate][tier4]") {
...
TEST_CASE("test_rental_income_credits_player_wealth", "[real_estate][tier4]") {
    ...
    RealEstateModule module;
    auto prop = make_test_property(
        1, PropertyType::residential, 0, 1, 200000.0f);  // owner_id = player_id = 1
    prop.rented = true;
    prop.tenant_id = 50;
    module.add_property(prop);
...
TEST_CASE("test_rental_income_credits_npc_capital", "[real_estate][tier4]") {
    ...
    RealEstateModule module;
    auto prop = make_test_property(
        1, PropertyType::commercial, 0, 42, 250000.0f);  // NPC owner_id = 42
    prop.rented = true;
    prop.tenant_id = 99;
    module.add_property(prop);
...
TEST_CASE("test_no_rental_income_for_unrented_property", "[real_estate][tier4]") {
    ...
    RealEstateModule module;
    auto prop = make_test_property(
        1, PropertyType::residential, 0, 42, 200000.0f);
...
TEST_CASE("test_multiple_rented_properties_accumulate_player_income", "[real_estate][tier4]") {
    ...
    auto prop1 = make_test_property(
        1, PropertyType::residential, 0, 1, 100000.0f);
    prop1.rented = true;
    prop1.tenant_id = 50;
    module.add_property(prop1);

    auto prop2 = make_test_property(
        2, PropertyType::commercial, 0, 1, 200000.0f);
    prop2.rented = true;
    prop2.tenant_id = 51;
...
TEST_CASE("test_transaction_above_threshold_flagged", "[real_estate][tier4]") {
    ...
    REQUIRE_THAT(RealEstateConstants::transaction_evidence_threshold,
                 WithinAbs(50000.0f, 0.01f));

    // Verify the constant is set correctly for use in transaction processing.
    PropertyListing prop = make_test_property(
        1, PropertyType::residential, 0, 100, 60000.0f);
...
TEST_CASE("test_transaction_below_threshold_not_flagged", "[real_estate][tier4]") {
    PropertyListing prop = make_test_property(
        1, PropertyType::residential, 0, 100, 40000.0f);
    REQUIRE(prop.market_value < RealEstateConstants::transaction_evidence_threshold);
}
...
TEST_CASE("test_commercial_tenant_assignment", "[real_estate][tier4]") {
    ...
    RealEstateModule module;
    auto prop = make_test_property(
        1, PropertyType::commercial, 0, 100, 250000.0f);
    prop.rented = false;
    prop.tenant_id = 0;
    module.add_property(prop);
...
TEST_CASE("test_residential_property_not_assigned_commercial_tenant", "[real_estate][tier4]") {
    ...
    RealEstateModule module;
    auto prop = make_test_property(
        1, PropertyType::residential, 0, 100, 150000.0f);
    prop.rented = false;
    prop.tenant_id = 0;
    module.add_property(prop);
...
TEST_CASE("test_business_already_has_premises_not_reassigned", "[real_estate][tier4]") {
    ...
    auto prop2 = make_test_property(
        2, PropertyType::commercial, 0, 100, 200000.0f);
    prop2.rented = false;
    prop2.tenant_id = 0;
    module.add_property(prop2);
...
TEST_CASE("test_execute_processes_all_provinces", "[real_estate][tier4]") {
    ...
    auto prop0 = make_test_property(
        1, PropertyType::residential, 0, 42, 100000.0f);
    prop0.rented = true;
    prop0.tenant_id = 50;
    module.add_property(prop0);

    auto prop1 = make_test_property(
        2, PropertyType::commercial, 1, 43, 200000.0f);
    prop1.rented = true;
    prop1.tenant_id = 51;
    module.add_property(prop1);
...
TEST_CASE("test_monthly_tick_updates_region_delta", "[real_estate][tier4]") {
    ...
    RealEstateModule module;
    module.add_property(make_test_property(
        1, PropertyType::residential, 0, 100, 200000.0f));
...
TEST_CASE("test_real_estate_constants", "[real_estate][tier4]") {
    REQUIRE_THAT(RealEstateConstants::residential_yield_rate,
                 WithinAbs(0.003f, 0.0001f));
    REQUIRE_THAT(RealEstateConstants::commercial_yield_rate,
                 WithinAbs(0.004f, 0.0001f));
    REQUIRE_THAT(RealEstateConstants::industrial_yield_rate,
                 WithinAbs(0.005f, 0.0001f));
    REQUIRE_THAT(RealEstateConstants::price_convergence_rate,
                 WithinAbs(0.05f, 0.0001f));
    REQUIRE(RealEstateConstants::convergence_interval == 30);
    REQUIRE_THAT(RealEstateConstants::criminal_dominance_penalty,
                 WithinAbs(0.15f, 0.0001f));
    REQUIRE_THAT(RealEstateConstants::laundering_premium,

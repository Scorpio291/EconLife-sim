...
TEST_CASE("society observe: the world spectrum (garden -> earthlike -> deathworld)",
    ...
    for (const auto& a : archs) {
        ...
        std::snprintf(label, sizeof(label), "%s (Class %.1f, bounty %.2f)", a.name,
                      static_cast<double>(world_class(a.hazard)), static_cast<double>(a.bounty));
    ...
    for (const auto& s : series) {
        if (s.year % 500 == 0) {
            ...
            std::printf(
                ...
                urban_pct, static_cast<double>(s.knowledge), s.era, s.soil_health, s.ghost_land,
                s.coal_remaining, s.political_stress, s.faction_deaths);
        }
    }
}
...
TEST_CASE("society observe: the historical climb (year each era is reached)",
    ...
    for (const auto& r : runs) {
        ...
        std::printf("\n=== %s (Class %.1f, bounty %.2f): era reached / year ===\n", r.label,
                    static_cast<double>(world_class(r.arch.hazard)),
                    static_cast<double>(r.arch.bounty));
        int prev_era = 0;
        float prev_k = 0.0f;
        ...
        for (const auto& s : series) {
            ...
            if (s.era != prev_era) {
                ...
                const double urban_pct = s.total_population > 0.0
                                             ? 100.0 * s.urban_population / s.total_population
                                             : 0.0;
                std::printf(
                    "  %s era %2d  @ year %5u  (+%4u yrs, knowledge %.0f, %+.2f/yr, pop %.0f, "
                    "spec %.0f%%, urban %.0f%%, capital/head %.0f, gini %.2f)\n",
                    s.era > prev_era ? "RISE" : "FALL", s.era, s.year, dy,
                    static_cast<double>(s.knowledge),
                    dy > 0 ? static_cast<double>(dk) / static_cast<double>(dy) : 0.0,
                    s.total_population, s.specialist_fraction * 100.0, urban_pct,
                    s.productive_capital_per_head, s.capital_gini);
                prev_era = s.era;
                prev_k = s.knowledge;
                prev_year = s.year;
            }
...
// ===========================================================================
TEST_CASE("society observe: does any REGION lose what it knew? (R6)", "[.society-regional-falls]") {
    constexpr uint32_t kNpcs = 200;
    constexpr uint32_t kYears = 13000;
    auto series = run_society_years(7, kNpcs, kYears, archetype_earthlike(),
                                    /*founding_hardiness=*/0.0f, /*fast_forward=*/true);
    ...
    for (const auto& s : series) {
        ...
        std::printf("  %5u | %13.0f | %13.0f | %13.0f | %.3f | %2d\n", s.year, s.knowledge_leader,
                    s.knowledge_laggard, s.knowledge_mean, ratio, s.era);
    }
    ...
    std::printf(
        "  DURING THE CLIMB:  deepest %.1f%% at year %u, in %d distinct episodes "
        "past a tenth\n",
        worst_climb * 100.0, climb_year, episodes);
    const auto& last = series.back();
    std::printf(
        "  final spread: leader %.0f, laggard %.0f (laggard holds %.1f%% of the "
        "frontier)\n",
        last.knowledge_leader, last.knowledge_laggard,
        last.knowledge_leader > 0.0 ? 100.0 * last.knowledge_laggard / last.knowledge_leader
                                    : 100.0);
    std::printf(
        "  If the ratio stays near 1.00 the world is still ONE civilisation with "
...
TEST_CASE("society observe: how bad does it ever get for ONE province?",
    ...
    for (const auto& r : runs) {
        ...
        std::printf("\n=== %s: the worst any ONE province ever had it (after year %u) ===\n",
                    r.label, kDawnEnds);
        std::printf(
            "  food:      worst surplus %.3f (year %u); %d years with some province "
            "under 1.0\n",
            worst_surplus, worst_surplus_year, hungry_years);
        std::printf("  soil:      worst %.3f of pristine\n", worst_soil);
        std::printf(
            "  stratum:   smallest non-farming share %.4f  (scattering is what makes a "
            "fall stick)\n",
            worst_spec);
        std::printf(
            "  war:       worst annual death fraction %.4f (year %u); %d years with any "
            "war at all\n",
...
// ===========================================================================
TEST_CASE("society observe: how many times does it fall? (the brief)", "[.society-cycles]") {
    constexpr uint32_t kNpcs = 200;
    constexpr uint32_t kYears = 13000;
    struct Run {
        const char* label;
    ...
    for (const auto& r : runs) {
        ...
        for (const auto& s : series) {
            ...
            if (!falling) {
                ...
                if (pop >= peak) {
                    ...
                    ++episodes;
                    std::printf(
                        "  fall %d: peaked %.0f at year %u, bottomed %.0f (-%.0f%%), "
                        "back by year %u (%u yrs); knowledge %.0f -> %.0f\n",
                        episodes, peak, peak_year, trough, 100.0 * (1.0 - trough / peak), s.year,
                        s.year - peak_year, peak_knowledge, static_cast<double>(s.knowledge));
                    falling = false;
                    peak = pop;
                    peak_year = s.year;
                    peak_knowledge = static_cast<double>(s.knowledge);
        ...
        if (falling) {
            ...
            const auto& last = series.back();
            std::printf(
                "  fall %d: peaked %.0f at year %u, bottomed %.0f (-%.0f%%), STILL DOWN "
                "at year %u; knowledge %.0f -> %.0f\n",
                episodes, peak, peak_year, trough, 100.0 * (1.0 - trough / peak), last.year,
                peak_knowledge, static_cast<double>(last.knowledge));
        }
        if (episodes == 0)
...
// ===========================================================================
TEST_CASE("society observe: does the economy actually industrialise?", "[.society-industrialise]") {
    constexpr uint32_t kNpcs = 200;
    constexpr uint32_t kYears = 13000;
    auto series = run_society_years(7, kNpcs, kYears, archetype_earthlike(),
                                    /*founding_hardiness=*/0.0f, /*fast_forward=*/true);
    ...
    std::printf(
        "  era | year  | non-farming | SUPPORTED | urban | capital/head | pop        |"
        " surplus\n");
    int prev_era = 0;
    for (const auto& s : series) {
        if (s.era == prev_era)
        ...
        std::printf("  %3d | %5u | %10.1f%% | %8.1f%% | %4.1f%% | %12.0f | %10.0f | %.3f\n", s.era,
                    s.year, s.cohort_specialist_share * 100.0, s.supported_share * 100.0, urban_pct,
                    s.productive_capital_per_head, s.total_population, s.mean_surplus);
    }
    std::printf(
...
TEST_CASE("society observe: knowledge at each era's historical year (F7 calibration)",
    ...
    const Target targets[] = {
        {2, "Bronze Age", -3300},        {3, "Iron Age", -1200},
        {4, "Classical", -550},          {5, "Medieval", 500},
        {6, "Early Modern", 1450},       {7, "Industrial", 1750},
...
TEST_CASE("society observe: transplant — soft vs native people on a harsh world",
    ...
    auto native =
        run_society_years(kSeed, kNpcs, kYears, harsh_fertile, /*founding_hardiness=*/0.0f);
...
TEST_CASE("society observe: who actually produces knowledge (mechanism audit)",
    ...
    for (uint32_t y = 0; y <= kYears; ++y) {
        if (y % 50 == 0) {
            ...
            const double actual =
                static_cast<double>(world.technology.knowledge_level) - last_level;
            std::printf(
                ...
                pop_total > 0.0 ? capital_stock / pop_total : 0.0, predicted, module_says,
                y == 0 ? 0.0 : actual / 50.0);
...
TEST_CASE("society observe: knowledge mechanism audit (fast, 20y)", "[.society-knowledge-quick]") {
    // Calibration/diagnostic companion to the knowledge trace. The trace shows the
    // RATE; this shows WHERE it comes from, so a stall can be attributed to a
    // mechanism instead of guessed at. It exists because a filter regression once
    // zeroed the scholar corps and left every world at era 1 for 13,000 years while
...
TEST_CASE("society observe: why is the dawn population not pressing on its land?",
    ...
    std::printf(
        "  year |    pop |  growth | surplus | spec%% | urban%% | soil  | topsoil | forest | "
        "fish/K       | fishers |"
        " era\n");
    for (size_t i = 0; i < series.size(); ++i) {
        ...
        if (i >= 500 && series[i - 500].total_population > 0.0)
            growth = 100.0 *
                     (std::pow(s.total_population / series[i - 500].total_population, 1.0 / 500.0) -
                      1.0);
        const double urban_pct =
            s.total_population > 0.0 ? 100.0 * s.urban_population / s.total_population : 0.0;
        std::printf(
            "  %5u | %6.0f | %+6.3f%% |  %.3f  | %4.0f%% | %5.0f%% | %.3f |  %.4f | %.4f |"
            " %.3f/%.3f | %7.0f | %2d\n",
            s.year, s.total_population, growth, s.mean_surplus, s.cohort_specialist_share * 100.0,
            urban_pct, s.soil_health, s.topsoil, s.forest, s.fish_stock, s.fish_capacity, s.fishers,
            s.era);
    }
}
...
TEST_CASE("society observe: inside the cycle", "[.society-cycle-detail]") {
    ...
    const int hi =
        std::min<int>(static_cast<int>(series.size()) - 1, static_cast<int>(worst_i) + 200);
    std::printf("\n  deepest fall %.0f%% bottoming at year %u\n", worst * 100.0,
                series[worst_i].year);
    std::printf(
        "  year |    pop |  d%%/yr | surplus | spec%% | statr | fit  | school |"
    ...
    for (int i = lo; i <= hi; ++i) {
        ...
        std::printf(
            "  %5u | %6.0f | %+6.2f | %7.3f | %4.0f%% | %.3f | %.2f | %6.2f |"
            " %5.2f | %6.0f | %5.2f | %.4f | %.4f | %.4f | %2d\n",
            s.year, s.total_population, g, s.mean_surplus, s.cohort_specialist_share * 100.0,
            s.nutrition, s.health, s.schooling, s.tech_food, s.productive_capital_per_head,
            s.food_store_years, s.political_stress, s.max_faction_deaths, s.max_war_deaths, s.era);
    }
}
...
TEST_CASE("society observe: does the cycle survive a finer stride?", "[.society-stride]") {
    ...
    for (uint32_t stride : {1u, 4u, 12u, 52u}) {
        auto series =
            run_society_years(7, kNpcs, kYears, archetype_earthlike(), 0.0f, false, stride);
        double peak = 0, trough = 1e18, worst = 0, run_max = 0;
        double k_max = 0, k_draw = 0;
        for (const auto& s : series) {
            peak = std::max(peak, s.total_population);
            if (s.total_population > 0)
                trough = std::min(trough, s.total_population);
        ...
        std::printf("  %8u | %7.0f | %7.0f | %9.0f%% | %7.0f | %7d | %9.0f | %9.0f%%\n", stride,
                    peak, trough, worst * 100.0, last.total_population, last.era,
...
TEST_CASE("society observe: the spread across seeds", "[.society-seeds]") {
    ...
    for (uint64_t seed : {1ull, 7ull, 42ull, 1000ull, 2024ull}) {
        ...
        const auto& last = series.back();
        std::printf(
            "  %5llu | %4d | %4d | %4d | %4d | %7d | %7.0f | %9.0f | %8.0f | %9.0f%% |"
            " %9.0f%% | %8d\n",
            static_cast<unsigned long long>(seed), first[2], first[3], first[4], first[5], last.era,
            last.total_population, static_cast<double>(last.knowledge), peak, worst * 100.0,
            k_draw * 100.0, falls);
    }
}

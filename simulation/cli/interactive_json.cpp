...
std::string tick_to_date(uint32_t tick) {
    ...
    char buf[32];
    std::snprintf(buf, sizeof(buf), "%04d-%02d-%02d", base.tm_year + 1900, base.tm_mon + 1,
                  base.tm_mday);
...
static const char* npc_role_str(NPCRole role) {
    switch (role) {
        ...
        case NPCRole::journalist:
            return "Journalist";
        case NPCRole::ngo_investigator:
            return "NGO Investigator";
        case NPCRole::community_leader:
            return "Community Leader";
        case NPCRole::union_organizer:
            return "Union Organizer";
        case NPCRole::criminal_operator:
            return "Criminal Operator";
        case NPCRole::criminal_enforcer:
            return "Enforcer";
        case NPCRole::fixer:
            return "Fixer";
        case NPCRole::bodyguard:
            return "Bodyguard";
        case NPCRole::family_member:
...
static const char* travel_status_str(NPCTravelStatus s) {
    switch (s) {
        case NPCTravelStatus::resident:
            return "resident";
        case NPCTravelStatus::in_transit:
            return "in_transit";
        case NPCTravelStatus::visiting:
            return "visiting";
        default:
            return "unknown";
    }
...
static const char* scene_card_type_str(SceneCardType t) {
    switch (t) {
        case SceneCardType::meeting:
            return "meeting";
        case SceneCardType::call:
            return "call";
        case SceneCardType::personal_event:
            return "personal_event";
        case SceneCardType::news_notification:
...
static const char* scene_setting_str(SceneSetting s) {
    switch (s) {
        case SceneSetting::boardroom:
            return "boardroom";
        case SceneSetting::private_office:
            return "private_office";
        case SceneSetting::open_plan_office:
            return "open_plan_office";
        case SceneSetting::factory_floor:
            return "factory_floor";
        case SceneSetting::warehouse:
            return "warehouse";
        case SceneSetting::construction_site:
            return "construction_site";
        case SceneSetting::laboratory:
            return "laboratory";
        default:
            return "other";
    }
...
static const char* calendar_type_str(CalendarEntryType t) {
    switch (t) {
        ...
        case CalendarEntryType::event:
            return "event";
        case CalendarEntryType::operation:
            return "operation";
        case CalendarEntryType::deadline:
            return "deadline";
...
static const char* business_sector_str(BusinessSector s) {
    switch (s) {
        ...
        case BusinessSector::services:
            return "services";
        case BusinessSector::real_estate:
            return "real_estate";
        ...
        case BusinessSector::media:
            return "media";
        case BusinessSector::security:
            return "security";
...
float compute_avg_spot_price(const WorldState& world) {
    if (world.regional_markets.empty())
        return 0.0f;
    float total = 0.0f;
...
nlohmann::json serialize_ui_state(const WorldState& world) {
    ...
    for (const auto& card : world.pending_scene_cards) {
        ...
        for (const auto& line : card.dialogue) {
            dialogue.push_back({{"speaker", npc_display_name(world, line.speaker_npc_id)},
                                {"text", line.text},
    ...
    for (const auto& entry : world.calendar) {
        ...
        calendar.push_back({{"id", entry.id},
    ...
    for (const auto& prov : world.provinces) {
        provinces.push_back({{"id", prov.id},
...
bool parse_and_enqueue_action(const nlohmann::json& cmd, WorldState& world) {
    ...
    if (action_type == "start_business") {
        ...
        if (sector_str == "manufacturing")
            sector = BusinessSector::manufacturing;
        else if (sector_str == "food_beverage")
            sector = BusinessSector::food_beverage;
        else if (sector_str == "services")
            sector = BusinessSector::services;
        else if (sector_str == "real_estate")
            sector = BusinessSector::real_estate;
        else if (sector_str == "agriculture")
            sector = BusinessSector::agriculture;
        else if (sector_str == "energy")
            sector = BusinessSector::energy;
        else if (sector_str == "technology")
            sector = BusinessSector::technology;
        else if (sector_str == "finance")
            sector = BusinessSector::finance;
        else if (sector_str == "transport_logistics")
            sector = BusinessSector::transport_logistics;
        else if (sector_str == "media")
            sector = BusinessSector::media;
        else if (sector_str == "security")
            sector = BusinessSector::security;
        else if (sector_str == "research")
            sector = BusinessSector::research;
        else if (sector_str == "criminal")
            sector = BusinessSector::criminal;

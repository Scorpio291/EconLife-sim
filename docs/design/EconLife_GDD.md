...
*Pass 5 Session 9 (Scenario Tests) applied: 3 GDD scenario stubs added (GDD-S1 Whistleblower Timing, GDD-S2 NPC Bankruptcy Triggers, GDD-S3 Opposition Formation Threshold)*
*Pass 3 Session 4 applied: Criminal Entry Conditions (§12.1)*
*Pass 3 Session 5 applied: Section 2 Setting prose written (Year 2000 start); companion document refs updated to current versions*
*Pass 3 Session 6 applied: TDD cross-reference resolution — obligation escalation (§10.4), opposition formation (§14.4), street reputation display (§12.1), tick step 22 (§21), evidence lifecycle (§10.2), informant mechanic (§12.2), trafficking threshold (§10.5), satisfaction derivation (§6.8), union V1 scope (§6.6), prison operational modifiers (§10.1), criminal dominance index (§12.12), burned identity dormant state (§12.6), journalist credibility (§10.3), price adjustment per-good (§8.2), election formula (§15), companion doc versions (§23), tick step additions 13b/26a/27/28 (§21)*
*Pass 3 Session 22 applied: §23 WorldGen scope promoted to V1; Procedural World Generation added to Companion Documents; companion TDD updated to v22*
*Pass 3 Session 23 applied: §23 weapons trafficking removed from exclusions; §16 social media + outlet ownership confirmed V1; §10.5 named consequence threshold constants added; §23 companion TDD updated to v23*
*Pass 5 Session 1 (Production Readiness Audit — GDD BLOCKER fixes: B1–B7) applied: §3 NPC Decision Engine Specification [CORE] (threshold gates + utility score models with factor definitions and decision category table), §6.8 Whistleblower channel selection formula (expected_value with trust/impact/risk parameters and defaults), §10.6 Prison mechanics specification (delegation efficiency 0.25, escalation multiplier 2.0, rival territory expansion, calendar lockdown), §7.8 Facility signal formulas (signal_score composite with power/chemical/traffic/noise/financial components and type-specific weights), §20 Regional conditions update formulas (infrastructure decay, inequality via Gini coefficient, stability recovery, criminal dominance index, addiction progression), §21 Consequence delay scheduling (BASE_DELAY table with 8 consequence types, random variance, awareness modifiers), §21 OPSEC profile enumeration and scrutiny reduction formula (minimal through paranoid profiles with costs and effectiveness rates)*
*Pass 5 Session 5 (PRECISION, INCONSISTENCY, SCOPE fixes: P1–P5, I1–I4, SC1–SC3) applied: §8.2 Market price adjustment per-good model (price_adjustment_speed field, one-tick lag), §3 NPC motivation weight system unification (normalized additive scoring, utility interface), §13.4 Trust recovery numeric bounds (recovery rate 2.0 pts, cap 0.75, floor 90 ticks), §8.2/§10.5 Market vs. consequence threshold relationship (composite exposure_score components), §6.8 NPC hire behavior specification (APPLICANT_COUNT_BASE, arrival window, offer expiration, quality distribution), §12.1 Criminal reputation field reconciliation (street_reputation = criminal_operator, single unified field), §10.6 Prison calendar management (visitor requests, legal counsel, delegation orders), §10.4 Obligation escalation vs. trust erosion parallelism (concurrent systems, escalation interval 30 ticks, erosion 0.5/tick), §10.2 Evidence decay rate formula (natural 0.002/tick, neutralization 3.0×, floor 0.10, batch every 7 ticks), §6.6 V1 union organizer actions (organize_workplace, recruit_member, public_campaign), §9.4 Education level drift formula (EDUCATION_DRIFT_RATE 0.001, target education function), §12.9 Infiltration V1/EX boundary (alternative identities V1, infiltration mechanics EX, Bootstrapper constraints)*
...
Globalization is at its ideological peak. NAFTA is six years old. The WTO is five. China's accession is pending, expected within the year. The policy consensus — across parties in most developed nations — is that open borders for goods and capital make everyone richer. Offshoring is presented as efficiency, not loss. This consensus will fray badly before the simulation ends.

The labor market is intact. Routine manufacturing employment is high. The automation wave of the 2010s and 2020s — the one that empties factory floors and bifurcates wages — hasn't arrived yet. Low-skill workers have more options than they will have by Era 4. The inequality that will come to define the later eras is present at the start, but it hasn't yet become the defining political fact of life in most nations.

Climate change is a recognized scientific concern but not yet a regulatory reality. The Kyoto Protocol was signed in 1997 but not ratified by major emitters. Regulations are light. The global CO₂ index begins at its Year 2000 baseline. The agricultural productivity redistribution — northern regions gaining as southern ones dry out — is an arc that develops across the simulation; it is not a starting condition but a consequence.

Surveillance infrastructure is rudimentary by later standards. CCTV is not ubiquitous outside airports and banks. Digital financial monitoring is largely manual — flagging suspicious transactions requires human review. The smartphone-enabled tracking infrastructure doesn't exist. The surveillance state that emerges in many nations by Era 4 is built during play, driven by political outcomes and player actions, not inherited from the starting world. Cryptocurrency does not exist. Digital payments run on card networks. Cash is still the primary medium for informal and criminal economies; its displacement is an R&D unlock, not a given.

The world the player starts in is not the world they will end in. The automation that empties factory floors, the surveillance architecture that makes criminal operations harder to hide, the climate redistribution that restructures agriculture and energy, the financial disruption that changes how money moves and hides — these are outcomes of the simulation, not its premises. They emerge from NPC technology investment, political cycles, climate accumulation, and the choices the player makes across 25 years of game time. The player does not inherit this world. They participate in building it.
...
Risk tolerance — how willing an NPC is to act on what they know, escalate a conflict, or expose themselves to consequence — is set at creation from background and accumulated experience. It is not fixed. Life continues.

Being victimized lowers it. An NPC who is robbed, assaulted, arrested, or burned in a criminal context has learned that bad outcomes happen when they overreach their protection. Being trained for competence raises it. Martial arts, security work, military service — these close the gap between perceived threat and the NPC's sense of their ability to handle it. The direction is not automatic: a high-revenge-motivated NPC who gets robbed may become angrier and more willing to act, while a high-stability-motivated NPC in the same situation retreats. The same event produces different changes in different people depending on what they want and how they're built.

Gaining dependents raises the cost of a bad outcome and lowers risk tolerance. Losing dependents, or reaching genuine desperation from prolonged scarcity, can push it upward — either through grief removing remaining caution, or through survival logic making inaction as costly as action. A person with nothing left to protect is a different kind of risk-taker than someone who never cared. Peer norms also matter: an NPC embedded in a network where risk-taking is normalized and visibly rewarded drifts upward over time. An NPC who watches a peer take a risk and lose publicly drifts downward.

Over time, absent new events, risk tolerance drifts back toward the NPC's personal baseline — but slowly, and never fully if major events have accumulated. This is why the witness who was too frightened to come forward in year one may be ready in year four. Their circumstances changed. The edges of their fear softened. Their calculation changed.
...
### NPC Decision Engine Specification [CORE]

NPCs make decisions through a **hybrid architecture**: threshold gates for critical decisions (whether to defect, whistleblow, escalate conflict) and utility scores for routine decisions (job selection, spending, movement, resource allocation). This section specifies both models.

**Threshold Gate Model (Critical Decisions)**

Critical decisions are evaluated as ordered condition checks. The first gate that evaluates to TRUE triggers the decision; gates are checked in priority order and stop on the first TRUE result.

Decision gates for whistleblowing (§6.8):
```
1. IF satisfaction < 0.35 AND memory contains ∈{witnessed_illegal_activity, witnessed_safety_violation, witnessed_wage_theft} with emotional_weight < −0.6 AND risk_tolerance > 0.4
   THEN → evaluate channel selection (see §6.8 Whistleblower channel selection)
2. IF no gate above → no whistleblowing action this tick
```

> Test: When a worker NPC meets all three whistleblower conditions (observed_violations ≥ 3, trust_in_employer < 0.3, risk_tolerance > 0.4), the NPC initiates whistleblower action within WHISTLEBLOWER_DECISION_WINDOW = 14 ticks, not instantaneously. The Delay Principle applies: action_tick = condition_met_tick + random(1, 14).
>
> Seed setup: Facility with 5 worker NPCs. Player commits 3 observable violations over 30 ticks. At least one worker has trust_in_employer < 0.3 and risk_tolerance > 0.5. Assert: whistleblower action occurs between tick 31 and tick 44 (not before tick 31).

Decision gates for criminal defection (when an operative is arrested):
```
1. IF cooperation_benefit_calculated > personal_liability_calculated AND arrest_credibility(target) < 0.5
   THEN → cooperate with law enforcement
2. IF mutual_incrimination_obligation > 0 (shared evidence suppression node exists)
   THEN → refuse cooperation (mutual destruction prevents either party speaking)
3. ELSE → roll random defection check (risk_tolerance-dependent base probability)
```

**Utility Score Model (Routine Decisions)**

For non-critical decisions, NPC action selection uses:

```
utility(action) = Σ(weight_i × factor_i)
```

| Factor | Range | Meaning | Application |
|---|---|---|---|
| `motivation_alignment` | [−1.0, 1.0] | How well this action serves the NPC's motivation profile | All decisions |
| `relationship_trust` | [−1.0, 1.0] | Directional trust toward the decision target (player, another NPC, institution) | Contact/engagement decisions |
| `risk_assessment` | [0.0, 1.0] | NPC's estimate of negative outcome probability | Decisions with exposure potential |
| `resource_availability` | [0.0, 1.0] | Can the NPC actually execute this action? | All decisions |
| `time_cost` | [−1.0, 0.0] | Weighted by NPC's schedule availability (negative utility) | Decisions requiring calendar commitment |
| `success_probability_estimated` | [0.0, 1.0] | NPC's (possibly imperfect) estimate of action success | Investigation, negotiation, influence decisions |

The NPC selects the action with highest utility. Ties are broken by recency (most recently considered action wins). Close-call utilities (within 0.1 of the maximum) add random variance (±0.05) to model decision uncertainty.

**Decision Categories and Assigned Models**

| Decision Category | Model | Examples | Weight Overrides |
|---|---|---|---|
| **Critical threshold** | Threshold gates (ordered) | Whistleblowing, defection, escalation of violence | Fixed; not configurable per NPC |
| **Routine economic** | Utility score | Job choice, business investment, product pricing | `motivation_alignment` weight = 0.4, `resource_availability` = 0.3 |
| **Social/relationship** | Utility score | Engagement offer acceptance, aid provision, relationship maintenance | `relationship_trust` weight = 0.5, `motivation_alignment` = 0.3 |
| **Investigative** | Utility score | Story pursuit (journalists), case advancement (regulators), cooperation (witnesses) | `success_probability_estimated` = 0.4, `risk_assessment` = 0.3 |
| **Movement/navigation** | Utility score | Location changes, social contact initiation, venue selection | `time_cost` = 0.3, `motivation_alignment` = 0.3 |

All constants are configuration-backed in `simulation_config.json → npc_decision_model`. Threshold gate conditions are fixed in code to preserve the "critical" semantics; utility weights may be tuned per NPC type (journalist, regulator, criminal_operator, etc.) via config.

**Motivation Weight Normalization [UNIFYING NOTE]**

All motivation weights across the NPC architecture operate under these unified principles:
- All motivation weights are normalized to the [0.0, 1.0] range.
- A composite motivation score is calculated as: `composite_motivation = Σ(weight_i × factor_i)` where `Σ(weight_i) = 1.0` across all applicable factors. This ensures the composite score also remains bounded to [0.0, 1.0].
- **Obligation urgency is multiplicative:** When an NPC has an overdue obligation (current_tick > deadline_tick), obligation_escalation_level scales the final composite motivation by up to 2.0×, multiplying the base composite score: `final_motivation = composite_motivation × (1.0 + obligation_escalation_level)`. This allows obligations to temporarily dominate an NPC's behavior without changing the underlying weight structure.
- This model unifies the three subsystems referenced in §10.4 (obligation escalation), §14.4 (opposition formation with grievance/leadership/resource weights), and other NPC decision contexts, ensuring consistent semantics across the entire motivation system.

**NPC Motivation Weight Systems Unified (P2 CLARIFICATION):**
All NPC motivation weights use NORMALIZED ADDITIVE scoring. Raw factors are computed per-system, then normalized to sum to 1.0 within each decision context. The utility score from §3 NPC Decision Engine provides the unified interface. System-specific weights feed into the utility model as factor inputs, not as independent scoring systems.
...
*[V1 scope: union organizer NPCs exist and targeting logic is active (see Scenario #7 — `scenario_union_targets_largest_employer`). "Thin" means: no strike mechanics, no collective bargaining system, no political integration in V1. The NPC is created, pursues the player's workforce as a target, and applies organizing pressure. Strike and bargaining resolution are Expansion scope — see Feature Tier List.]*

**V1 Union Organizer Actions (SC1 CLARIFICATION):**
V1 Union Organizer has three available actions: (1) organize_workplace — increases worker solidarity at target facility (+0.05 per action, max 1.0), (2) recruit_member — converts worker NPC to union member (probability based on worker grievance), (3) public_campaign — increases regional awareness of labor conditions (+0.02 regional_labor_awareness per action). Strike and collective bargaining are EX.
...
**Hiring Mechanics Specification:**

Applicant pool size is calculated as:
```
applicant_count = base_rate × (wage_premium_factor + reputation_bonus_factor) × regional_unemployment_factor
```

| Parameter | Meaning | Default/Notes |
|---|---|---|
| `base_rate` | Baseline applicants per tick at market-rate wage | 3 applicants per tick |
| `wage_premium_factor` | (posted_wage - regional_market_wage) / regional_market_wage; multiplies base | Range [0.5, 2.0]; wages 50% above market attract 2× more applicants |
| `reputation_bonus_factor` | Player's employer reputation modifier | Range [0.5, 1.5]; based on previous worker satisfaction and retention |
| `regional_unemployment_factor` | Regional unemployment rate / baseline | Range [0.5, 2.0]; high unemployment increases applicant pool |
| **Offer Expiration** | Job offer remains available to applicant | 7 ticks; applicant declines if not accepted within window |
| **Quality Distribution** | Applicant skill/experience distribution | normal distribution with mean = regional_education_level (or job-tier equivalent), stddev = 0.15 |
...
**NPC Hiring Behavior Specification (P5 CLARIFICATION):**
Job posting generates APPLICANT_COUNT_BASE (default: 3) + random(0, APPLICANT_VARIANCE) applicants. Applicants arrive over APPLICANT_ARRIVAL_WINDOW = 7 ticks. Offer expires after OFFER_EXPIRATION_TICKS = 14 ticks. Quality distribution: uniform within regional education_level ± 1 tier. Premium channels (headhunter, university) add +1 quality tier but cost 3× base posting.

**Day-to-day management:** Workers have satisfaction (a float 0.0–1.0), which is affected by: pay relative to market (the player can see the regional wage market and set compensation accordingly), working conditions (determined by facility design and management quality), how the player treats them in direct interactions, job security signals (is the business visibly healthy?), and what they witness at work. Satisfaction affects productivity, loyalty, and retention. <!-- C3 fix --> Satisfaction is derived each tick from the NPC's memory entries about their employment situation — it is not a cached field. Full derivation model: TDD §4 (NPC Architecture).
...
**Whistleblower risk:** A worker NPC becomes whistleblower-eligible when all three conditions hold: satisfaction < 0.35 AND the worker's memory log contains at least one entry of type ∈ {`witnessed_illegal_activity`, `witnessed_safety_violation`, `witnessed_wage_theft`} with emotional\_weight < −0.6 AND the worker's risk\_tolerance > 0.4. The whistleblower-eligible state is checked by the investigator engine each tick and may trigger a whistleblowing action based on the worker's motivation model. The memory type determines which authority is contacted: `witnessed_illegal_activity` → law enforcement NPC; `witnessed_safety_violation` or `witnessed_wage_theft` → regulator or journalist NPC (resolved by expected_value in the behavior engine). This is the threshold referenced in Section 22's contextual introduction examples.

**Whistleblower channel selection formula:**

When a whistleblower-eligible worker chooses to act, they evaluate available channels (law enforcement, regulator, journalist) using expected value analysis. For applicable channels (determined by memory entry type), the worker calculates:

```
expected_value(channel) =
    trust_in_institution(channel) × perceived_impact(channel)
    - perceived_risk(channel) × risk_aversion(npc)
```

| Parameter | Meaning | Range | Default by Channel |
|---|---|---|---|
| `trust_in_institution(channel)` | NPC's assessment of institutional trustworthiness and competence | [0.0, 1.0] | LE: 0.6, Regulator: 0.5, Journalist: 0.4 |
| `perceived_impact(channel)` | How likely this channel will create meaningful consequence for the player | [0.0, 1.0] | LE: 0.7, Regulator: 0.5, Journalist: 0.8 |
| `perceived_risk(channel)` | NPC's assessment of personal danger from retaliation | [0.0, 1.0] | LE: 0.3, Regulator: 0.4, Journalist: 0.5 |
| `risk_aversion(npc)` | 1.0 − risk_tolerance; NPC-specific | [0.0, 1.0] | Derived from NPC model |

The worker selects the channel with the highest expected value. The defaults reflect base institutional credibility; they may be modified by NPC experience (if law enforcement has failed them before, trust_in_institution(LE) decreases) and player reputation (a player known for violence increases perceived_risk for all channels).

> [SCENARIO: scenario_whistleblower_emerges_on_eligible_conditions]
> Test: A worker NPC with all three whistleblower-eligible conditions satisfied (satisfaction < 0.35, qualifying memory entry with emotional_weight < −0.6, risk_tolerance > 0.4) eventually contacts investigators; a worker missing any one condition does not.
...
> Seed setup: Worker NPC with witnessed_illegal_activity memory entry (emotional_weight = −0.7), risk_tolerance = 0.5, motivation includes financial grievance; player fires worker at tick 0.
...
**Facility Signal and Detection Risk Specification**

Each criminal facility generates a composite signal score based on observable operational characteristics. Law enforcement uses this signal score to assign detection probability. The signal score is facility-type-specific; legitimate facilities generate signals only on `visibility_scope = concealed` activities.

**Signal Score Calculation:**

```
signal_score(facility) = Σ(signal_weight[component] × component_value)
```
...
|---|---|---|---|
| `power_draw` | Normalized monthly electricity consumption | [0.0, 1.0] | Drug labs abnormally high; monitored by utility companies |
| `chemical_waste` | Volume/type of hazardous waste generation | [0.0, 1.0] | Drug synthesis produces distinctive waste streams |
| `foot_traffic` | Normalized visitor/personnel frequency | [0.0, 1.0] | High turnover indicates distribution point |
| `noise` | Acoustic signature (machinery, extraction fans, etc.) | [0.0, 1.0] | Processing equipment generates distinct sounds |
| `financial_anomaly` | Normalized ratio of cash revenue to declared purpose | [0.0, 1.0] | Money laundering operations show revenue mismatches |

**Signal weights by facility type:**

| Facility Type | power_draw | chemical_waste | foot_traffic | noise | financial_anomaly |
|---|---|---|---|---|---|
| Drug production lab | 0.35 | 0.40 | 0.15 | 0.05 | 0.05 |
| Distribution warehouse | 0.05 | 0.05 | 0.50 | 0.10 | 0.30 |
| Money laundering front | 0.10 | 0.05 | 0.20 | 0.10 | 0.55 |
| Smuggling waypoint | 0.10 | 0.15 | 0.30 | 0.15 | 0.30 |
| Trafficking facility | 0.15 | 0.10 | 0.40 | 0.20 | 0.15 |

**Detection Probability Formula:**

```
detection_probability(facility) =
    sigmoid(signal_score - detection_threshold(region))
    × (1.0 - opsec_reduction(facility))
```

Where:
- `sigmoid(x) = 1 / (1 + e^−x)` converts the signal-threshold difference into a probability [0.0, 1.0]
- `detection_threshold(region)` is a per-region baseline set during world generation (default: 0.4; can range from 0.2 in high-corruption jurisdictions to 0.6 in well-policed regions)
- `opsec_reduction(facility)` is the facility's current OPSEC effectiveness (see §21 OPSEC Profiles; range [0.0, 0.75])

Each tick, the investigation engine rolls a random check: if `random() < detection_probability(facility)`, law enforcement detects the facility and adds it to an investigator queue.
...
Each traded good has a **regional spot price** that adjusts each tick toward a calculated equilibrium at a configurable adjustment rate (global default: `config.price_adjustment_rate = 0.10` in economy.json; overridable per-good via the `price_adjustment_rate` field in the goods CSV). Financial assets adjust quickly; housing adjusts slowly; commodities at medium speed. This stickiness is the source of arbitrage opportunities and meaningful price signals.

**V1 Price Adjustment Clarification:** The per-good `price_adjustment_rate` from goods.csv controls how quickly a good's regional spot price converges toward its calculated equilibrium each tick. Formula: `price(t+1) = price(t) + (equilibrium_price - price(t)) × price_adjustment_rate`. The one-tick lag mentioned below applies only to supply/demand data visibility (information lag), not to the price adjustment mechanism itself. Each good adjusts its price independently according to its configured rate.
...
**Market Price Adjustment Per-Good Model (P1 CLARIFICATION):**
Price adjustment uses PER-GOOD rates. Each good has a `price_adjustment_speed` field (default: 1.0). Goods with higher speed approach equilibrium faster. Speed 0.5 = adjusts halfway per tick. Speed 2.0 = overshoots slightly then corrects. All goods use a one-tick lag (price at tick t reflects supply/demand at tick t-1).
...
> Test: When an NPC-owned business has cash < monthly_operating_cost for 3 consecutive monthly evaluations (90 ticks), the business enters bankruptcy state. monthly_operating_cost = sum of (worker_wages + facility_maintenance + input_costs) per 30 ticks.
>
> Seed setup: NPC business with monthly_operating_cost = 10,000. Set cash = 8,000 at tick 0 with zero revenue. Assert: bankruptcy triggers at tick 90 (third consecutive monthly shortfall), not at tick 30 (first shortfall).

### 8.8 The Informal Economy

The informal economy is not a separate layer — it is the same simulation running under different legality and visibility rules. Black market prices respond to formal market conditions because they share the same supply/demand model: smuggling is more profitable when tariffs are high, underground labor is more available when formal unemployment is high. The architectural distinction between formal and informal is `NPCBusiness.criminal_sector` (which market price signal the business reads) and `visibility_scope` (who can observe its activities) — not a separate supply chain, NPC model, or production system. A drug lab and a pharmaceutical plant are both `NPCBusiness` records running the same recipe system; the drug lab has `criminal_sector = true` and `visibility_scope = concealed`. See Technical Design §5 for the `VisibilityScope` enum and the `criminal_sector` / `VisibilityScope` canonical rules.
...
**Education Level Drift Formula (SC2 CLARIFICATION):**
education_level(t+1) = education_level(t) + EDUCATION_DRIFT_RATE × (target_education - education_level(t)) where target_education = f(regional_gdp_per_capita, education_investment, university_presence). EDUCATION_DRIFT_RATE = 0.001 per tick. education_investment comes from government spending allocation. university_presence is binary (0 or 1), adding +0.5 to target.
...
Evidence lives in the world attached to specific NPCs or locations. Each token has an `actionability` float (1.0 = fully actionable; floor = `actionability_floor`, config-backed, default 0.10). Actionability decays continuously via the DeferredWorkQueue — the decay rate is config-backed and accelerates 5× when the holder's credibility is damaged. Evidence never fully disappears (the floor is not zero — cold cases are real), but low-actionability evidence is much harder for investigators to use. Running out the clock is a real but incomplete strategy: old evidence remains on file at floor actionability. Active neutralization paths (buy, destroy, discredit, bury, leverage) reduce actionability faster than natural decay. Full decay formula in TDD §6.

**Evidence Decay Rate (I4 CLARIFICATION):**
NATURAL_DECAY_RATE = 0.002 per tick (baseline). Active neutralization: decay_rate = NATURAL_DECAY_RATE × NEUTRALIZATION_MULTIPLIER (default: 3.0) = 0.006 per tick. Floor: EVIDENCE_ACTIONABILITY_FLOOR = 0.10 (evidence never fully disappears). Decay is applied in batch every 7 ticks: batch_amount = rate × 7.
...
**Journalists** are individual significant NPCs with investigation specializations, funding sources, risk tolerances, and career motivations. A financial journalist doesn't pursue labor violations. A labor reporter doesn't follow money. Investigation meters fill over time when the player is a high-profile target in their beat. Critically: fired journalists don't disappear — they take their notes somewhere else and are now specifically hostile. Journalist credibility governs story impact: a journalist is assessed as non-credible (`is_credible() = false`) when `public_credibility < 0.3`, or when their memory log contains a `credibility_damaged` entry, or when it contains an `accepted_player_payment` entry. Non-credible journalists can still publish, but story impact is reduced and law enforcement does not treat the publication as an investigation trigger. Full credibility rules: TDD §6.
...
**Obligation escalation:** What people ask for in return escalates based on creditor motivation and the player's visible wealth — not randomly. An obligation node progresses through four status tiers: **open** (owed but not yet demanded) → **escalated** (demand has grown beyond the original value) → **critical** (demand has reached 3× original, consequence queued) → **hostile** (creditor takes unilateral action via their expected_value engine).

**Overdue open obligations and concurrent trust erosion:** An **overdue_open_obligation** is any obligation where `current_tick > deadline_tick AND status = open` (not yet satisfied or renegotiated). Obligation escalation and trust erosion are concurrent but independent processes:
- **Escalation:** Each tick an obligation remains overdue, `escalation_level += ESCALATION_RATE_PER_TICK` (default: 0.01 per tick; escalation_level can grow beyond 1.0). Escalation rate per tick = creditor's dominant motivation weight × `escalation_rate_base` × (1 + wealth_factor), where wealth_factor scales with the player's `visible_net_worth` — wealthier players face steeper demand growth because creditors read capacity to pay.
- **Trust erosion:** Each tick an obligation remains overdue, `trust_score -= TRUST_EROSION_PER_OVERDUE_TICK` (default: 0.005 per tick). This erodes the creditor's trust in the player independent of the escalation of the obligation demand itself.

A politician accepts a campaign contribution, then asks for a hire, then for influence over a regulatory decision, then for something that crosses into criminal exposure. Each step seems incremental. The cumulative commitment is not. All escalation and erosion rate constants are config-backed (see `simulation_config.json → obligations`). Full algorithm in TDD §7.

**Obligation Escalation vs. Trust Erosion (I3 CLARIFICATION):**
Escalation and erosion are CONCURRENT parallel systems. Escalation increases demand severity. Erosion decreases relationship trust. Both trigger from the same event (overdue obligation). 'Overdue' = obligation not fulfilled within deadline_ticks. Escalation advances one level per ESCALATION_INTERVAL = 30 ticks. Trust erodes at TRUST_EROSION_PER_TICK = 0.5 during overdue period.
...
Reaching significant net worth → a financial journalist who was already covering wealth stories adds the player to their watchlist and begins building a profile. Controlling 40%+ of a regional market → the competition regulator, whose job is to monitor market concentration, opens a preliminary inquiry. Becoming the largest regional employer → union organizers, who were already working the region, identify the player's workforce as their highest-value target. Winning a national election → foreign intelligence services begin standard monitoring of a new head of state. Owning a media outlet → political opponents begin investigating the ownership for conflicts of interest. Entering the drug distribution market at scale → narcotics investigators, already working the region, elevate the player to a priority target. Destabilizing a foreign political situation → international bodies and foreign intelligence services add the player to their active monitoring lists.

**Consequence threshold named constants** (all in `simulation_config.json → consequences`; P-05 resolution):

| Threshold | Constant | Value | Notes |
|---|---|---|---|
| "Significant net worth" | `consequences.journalist_watchlist_net_worth` | 500,000 | Game currency units; ~10× monthly legitimate business revenue at V1 province scale |
| "40%+ regional market" | `consequences.antitrust_market_share_threshold` | 0.40 | Fraction of provincial supply for a single good |
| "Largest regional employer" | `consequences.union_target_employer_threshold` | 1 | Player must be the single largest employer by headcount in the province |
| "Drug market at scale" | `consequences.drug_narcotics_priority_threshold` | 0.15 | Player share of provincial drug distribution volume |
| "Destabilizing foreign situation" | `consequences.foreign_destabilization_stability_floor` | 0.30 | Foreign province `political.stability` below this AND player has ≥1 causal consequence link |
...
**Market Influence vs. Consequence Thresholds [CLARIFICATION NOTE]**

Two threshold systems operate independently: §8.2 defines **market influence thresholds** (0.20, 0.50, 0.70) which measure `market_share` — the player's fraction of regional supply for a specific good. §10.5 defines **consequence thresholds** which include exposure-related triggers like "40%+ of regional market" (constant `consequences.antitrust_market_share_threshold`). These measure different dimensions:
- Market influence thresholds track supply concentration and affect equilibrium price calculation mechanics.
- Consequence thresholds trigger institutional attention (regulators opening inquiries, etc.) based on observable economic position.
- A player can achieve high market share (0.70) while maintaining low exposure_score (0.10) through effective OPSEC and political protection. Both dimensions must be tracked independently. Market dominance alone does not guarantee consequence triggers if investigation scrutiny is suppressed.

**Market vs. Consequence Threshold Relationship (P4 CLARIFICATION):**
Market influence thresholds and consequence triggers operate on DIFFERENT scales. Market share (0.0-1.0) measures supply control. Consequence triggers use composite exposure_score (0.0-1.0) from evidence + visibility + market power. The market share at 0.40 feeds into exposure_score as one component (weight: 0.30), not as a direct trigger.
...
**Prison:** Prison is not game-over. It is a heavily constrained operational state. The player character is physically confined. Existing delegation assignments persist — NPC managers continue handling their delegated functions at reduced efficiency. No new delegation assignments can be made and no new hires can be authorized during incarceration. Obligations continue accumulating and creditors can escalate; the player cannot take new obligation actions while inside. All external communication runs through monitored prison channels (scheduled phone calls, letters) — no secure or covert communications. Rivals probe. The player can interact with other inmates (some high-value criminal NPC contacts serve time in the same facility), manage health, and conduct limited legal communications through their attorney.
...
**Prison Mechanics Specification**

During incarceration, the following operational rules apply:

| Mechanic | Effect | Value / Formula |
|---|---|---|
| **Delegation Efficiency** | NPC managers retain reduced control; player retains limited oversight | `delegation_efficiency_multiplier = 0.25` (managers operate at 25% of normal effectiveness; operations degrade rapidly) |
| **Calendar State** | All scheduled external meetings and engagements cancelled; calendar locked | Pre-existing meetings are cancelled immediately upon incarceration. New external engagements cannot be scheduled. Only prisoner-specific events available: visitor_requests (max 1 per 7 ticks), lawyer_consultation (unlimited). Delegation orders use the lawyer communication channel with 3-tick delay. |
| **Visitor Requests** | Only mechanism for scheduled external contact | Visitor events queue; prisoner can accept/decline; monitored prison channels only |
| **Obligation Escalation** | Creditors continue escalating; player cannot satisfy obligations from inside | `obligation_escalation_rate_multiplier = 2.0` (escalation proceeds 2× faster); player cannot make payments or negotiate |
| **Rival Behavior** | Rivals receive `player_incarcerated` event | Triggers `territory_expansion_evaluation` with `aggressiveness_threshold` lowered by 0.3 (rivals more likely to attack during incarceration window) |
| **Revenue Generation** | Legal businesses continue generating revenue but at reduced efficiency | Delegated managers operate at the 0.25 multiplier; criminal operations must be wound down or abandoned (cannot be actively managed from inside) |
| **Health State** | Prison conditions affect character health; can be managed through commissary purchases | Health degradation rate increases 1.5× from baseline; healthcare access limited; violence risk from inmates |

The prison state is mechanically a transient constraint state, not a game-over condition. The player retains strategic decision-making via letter/phone contact with outside contacts and legal team, but operational control is severely degraded. The pressure from escalating obligations and rival expansion while unable to respond creates the mechanical core of the imprisonment experience.

**Prison State vs. Calendar Management (I2 CLARIFICATION):**
During incarceration: ALL scheduled meetings are cancelled. Calendar interactions available: (1) visitor_request (requires approval, 1 per 7 ticks max), (2) legal_counsel (unlimited, via legal representative NPC), (3) delegation_order (reduced effectiveness per §10.6 prison mechanics). All other calendar event types are BLOCKED until release.
...
**Architectural note:** The criminal economy is not a separate simulation. It is the same supply chain stack (Section 8), NPC motivation model (Section 3), financial infrastructure (Section 8.2), and evidence system (Section 10.2) — with two overlays applied. First, `criminal_sector = true` routes price signals to the informal market layer. Second, activities default to `visibility_scope = concealed`, meaning they require active investigation to surface rather than being observable through audit, public record, or regulatory inspection. The criminal path is a behavioral mode with different access gates and visibility rules, not a parallel economy running alongside the legitimate one.

### Criminal Entry Conditions [V1]

The following table specifies the minimum conditions required to initiate each criminal operation type. Conditions are evaluated at the moment of initiation attempt — they are not monitored passively or indicated in the UI. If conditions are not met when the player attempts to initiate (via scene card or contact interaction), the contact declines or the opportunity does not arise.

| Operation type | Required contact | Reputation minimum | Knowledge requirement |
|---|---|---|---|
| Street drug distribution | ≥1 NPC with role ∈ {`criminal_operator`, `fixer`}, trust > 0.4 | `street_reputation > 20` | Memory entry: `criminal_introduction` for a drug supply source |
| Drug manufacturing | ≥1 NPC with role ∈ {`criminal_operator`, `chemist`}, trust > 0.5 | `street_reputation > 35` | Memory entry: `supply_source` for precursor chemicals; player controls or has access to a securable facility |
| Money laundering | ≥1 NPC with role ∈ {`accountant`, `lawyer`, `banker`}, trust > 0.5 | `street_reputation > 15` | Player owns or controls a legitimate business with revenue > $50,000/month (game currency) |
| Criminal protection | ≥2 NPCs with role = `criminal_operator` in same province, trust > 0.4 each | `street_reputation > 40` | Memory entry: at least one other criminal operation in the province (witnessed or via intelligence) |
| Illegal weapons trade | ≥1 NPC with role = `weapons_supplier`, trust > 0.6 | `street_reputation > 45` | Memory entry: `weapons_source` |
| Human trafficking | **[EX — not V1]** | — | — |

**Evaluation logic:** There is no UI indicator that conditions are close to being met. The player infers readiness from their relationships, activity, and accumulated intelligence. Conditions are checked at the moment the player attempts to initiate the operation — not continuously evaluated in the background.

**Criminal Reputation Fields Reconciliation (I1 CLARIFICATION):**
`street_reputation` and `criminal_operator` are the SAME underlying field. Display name 'street_reputation' (0-100 scale) is the player-facing label. Internal field name 'criminal_reputation' is used in NPC structs. Both reference the same float value. No separate scales exist.

**`street_reputation` and criminal_operator reputation (UNIFIED FIELD):** `street_reputation` is the player-facing display name for the `criminal_operator.reputation_score` field. Both the player and NPC criminal_operators use the same field. Internal range: −1.0 to 1.0 (negative = hostile to criminal networks, zero = unknown, positive = trusted criminal contact). Display value = internal × 100. Character creation starts at internal 0.0 (display: 0); maximum = 1.0 (display: 100). The name differs for thematic reasons only (player sees "street reputation," NPC records show "criminal_operator reputation_score"), but they are the same underlying attribute tracked on the same reputation model. All criminal entry conditions (§12.1), criminal NPC relationship calculations, and cross-NPC criminal network propagation use this single unified field. It is built through: making criminal contacts, completing favors for criminal NPCs, being known within criminal networks (word propagates through `criminal_operator` relationship graphs). It is damaged by: cooperating with law enforcement, high-profile legitimate public activities that raise public profile without equivalent street activity. `street_reputation` is independent of `public_business`, `public_political`, and `public_social` — a player can have high public reputation and low street reputation simultaneously, or vice versa. The same mechanics apply to NPC criminal_operator reputation — the player is not a special case.
...
**The informant problem:** Every person in the criminal network is a potential informant the moment they are arrested. Cooperation value to law enforcement is an extremely powerful incentive. The player manages this through the same NPC mechanics that govern all relationships — there is no separate "loyalty" field. An operative's likelihood of defecting is governed by their `risk_tolerance`, their `trust` relationship score with the player, and whether mutual incrimination obligation nodes exist. Mutual incrimination is implemented as `FavorType::evidence_suppressed` or `FavorType::whistleblower_silenced` obligation nodes — both parties hold memory entries of the exchange, and both would be implicated if either talks. Compartmentalization means ensuring operatives' memory logs contain no entries naming other operatives — they can only give up what they witnessed. Violence to deter cooperation generates new Exposure and dramatically escalates LE response. These mechanics apply identically to NPC criminal operators managing their own networks — the player is not a special case.
...
**Operational Security:** All businesses manage information exposure — a pharmaceutical company guards its research pipeline, a manufacturer hides its supplier relationships, a hedge fund protects its trading strategy. OPSEC is not a criminal-exclusive concern; it is a universal property of any actor with information worth protecting. What the criminal path adds is that the activity itself must be hidden from law enforcement, not just from competitors. The `visibility_scope` of a criminal activity defaults to `concealed` — observable only through active investigation — whereas a legitimate trade secret sits at `internal` and a patent filing moves from `internal` to `public` on filing date. The criminal operator's specific OPSEC tools: compartmentalization (no single operative knows the full picture); communications discipline (nothing incriminating in writing, coded language, physical meetings for sensitive discussions); physical security (surveillance detection routes, dead drops); front organizations (all activity attributed to entities with plausible alternative explanations); non-traceable personnel (workers recruited through multiple intermediaries).
...
**Alternative identities:** The player can establish and maintain alternative identities with clean documentation. All actions taken under an alternative identity attach to that identity's evidence record, not the real identity's. When an alternative identity is burned, its evidence record enters a dormant state — all evidence tokens generated under that identity have `identity_linked = false` set against the player's real identity. The tokens exist with their normal `actionability` float intact and retain evidentiary value against the burned identity. They become actionable against the real player only when an investigator discovers the identity link — through a surviving network member, forensic document analysis, or a rival providing the connection. Identity-link discovery is a distinct investigative action, not automatic. Every burned identity is a permanent latent liability.
...
**[V1/EX BOUNDARY] Alternative Identities vs. Infiltration (SC3 CLARIFICATION):**
Alternative identities (creating false documents, maintaining cover stories) are V1. Infiltration mechanics (embedding an agent inside another organization, gaining insider access, extracting information) are EX. The Bootstrapper MUST NOT generate infiltration action handlers during V1 implementation. V1 alternative identities are limited to: identity_creation, identity_maintenance, identity_exposure_risk.
...
Criminal dominance degrades every simulation parameter in the affected region: property values fall, legitimate businesses face protection costs or refuse to operate entirely, public services deteriorate as tax revenue falls, educational outcomes decline, health outcomes decline, social trust collapses. `criminal_dominance_index` is a `float [0.0, 1.0]` field on `RegionConditions`, updated in tick step 20; it emerges from criminal organization territorial control expansion and is not set by the player directly.
...
**Trust-based Influence** is the hardest to rebuild. An NPC who reduced their trust based on betrayal has a memory of the specific event. Rebuild requires: **(1) Time:** minimum 180 ticks (6 in-game months) before any trust recovery begins; **(2) Positive interaction rate:** +0.01 trust recovery per tick of demonstrable positive interaction (cooperation, fair dealing, successful collaboration); **(3) Asymptotic ceiling:** recovery reaches maximum at 70% of the pre-betrayal trust_cap, meaning a permanently 30% reduction in that NPC's trust ceiling (this permanent gap is not recoverable through further interaction). Some trust relationships are permanently partially damaged — an NPC who was seriously harmed may never restore to pre-harm trust regardless of subsequent behavior. This permanent partial loss is by design.
...
**Trust Recovery Numeric Bounds (P3 CLARIFICATION):**
Trust recovery rate: TRUST_RECOVERY_PER_POSITIVE_INTERACTION = 2.0 points (on 0-100 scale). Minimum interactions for measurable recovery: 5. Time floor: 90 ticks minimum before trust can exceed 50% of pre-betrayal level. Recovery cap: trust can recover to at most 75% of pre-betrayal level (TRUST_RECOVERY_CAP = 0.75). Full trust recovery is NEVER possible after betrayal.
...
The world generates organized opposition proportional to the gap between the player's power and the grievances they've created. A formal opposition organization forms only when the community has reached **stage 6 (sustained_opposition)** — the seventh and final stage of the community escalation ladder. Stage 6 requires all three of its threshold conditions to be simultaneously met: grievance ≥ 0.85, a qualifying leadership NPC present, and resource_access ≥ 0.35. These three conditions are not independent from the stage system — they are the stage 6 conditions. Earlier convergence of grievance + leadership + resources without reaching the stage 6 thresholds does not trigger formal opposition formation; it drives the community toward stage 6 faster. When stage 6 is reached, a founding NPC is selected by highest combined (ideology + power + revenge motivation × social_capital × risk_tolerance), and an OppositionOrganization is created. Full stage threshold values in TDD §14.
...
> Test: When community_grievance reaches 0.85 in a province where a Significant NPC with role=community_leader exists AND that NPC has resources AND motivation=ideological with risk_tolerance > 0.5, an opposition movement forms. Grievance accumulates from player actions at province-specific rates.
>
> Seed setup: Province with community_grievance = 0.6. Community leader NPC present with all prerequisites. Player performs 5 actions that each add +0.05 grievance per tick. Assert: opposition forms when grievance crosses 0.85 (after ~5 ticks of player actions), not at the initial 0.6. Control: remove community_leader NPC and verify opposition does NOT form even at grievance 0.95.
...
**Election resolution:** Elections are resolved probabilistically. The formula: weighted demographic vote share (each group's support × `turnout_propensity`, summed) + resource modifier (campaign spending advantage via `tanh`, capped ±15%) + event modifiers (scandal and endorsement events accumulated during the campaign, capped ±20%) + random variance seeded at election-open. No outcome is mechanically guaranteed — which is what makes pre-election relationship building matter. A player who spent five years building genuine community trust before announcing has a different foundation than one who bought their coalition two months before polling day. Full formula: TDD §15.
...
12. Update facility signals and detection risk — signal composites recomputed for all facilities (criminal and legitimate); LE filters to `criminal_sector = true`; regulators read all facilities via `RegulatorScrutinyMeter`; surveillance coverage and informant network status update (TDD §16)
...
13b. Run regulator scrutiny mitigation — apply `scrutiny_mitigation` OPSEC if-branch; reduce active scrutiny scores based on current OPSEC profile (Design Decision A-01; TDD §12)

**OPSEC Profile Enumeration and Scrutiny Reduction:**

| Profile | Description | Scrutiny Reduction | Overhead Cost (% of revenue) | Implementation Time |
|---|---|---|---|---|
| **minimal** | No active countermeasures; operations transparent | 0.0 | 0% | N/A |
| **basic** | Shell companies, separate bank accounts, cash-intensive operations | 0.15 | 2–5% | 14 days |
| **moderate** | Multiple intermediaries, mixed cash/electronic transactions, front business layer | 0.35 | 8–15% | 30 days |
| **extensive** | Full compartmentalization, distributed supply chain, dedicated money laundering front(s), multiple identities | 0.55 | 20–30% | 60 days |
| **paranoid** | Total operational security, maximum compartmentalization, redundant infrastructure, counter-surveillance measures, internal security apparatus | 0.75 | 35–50% | 90 days |

Profiles are not automatically selected; the player (or criminal NPC) chooses a profile when establishing or expanding criminal operations. Profiles can be upgraded over time, but downgrading requires active dismantling (time and risk). Higher profiles increase survival likelihood but reduce profitability and require continuous maintenance resources.

**Scrutiny Reduction and Active Scrutiny Formula:**

```
active_scrutiny_score(t+1) =
    active_scrutiny_score(t) × (1 - SCRUTINY_NATURAL_DECAY)
    - opsec_reduction(profile) × OPSEC_EFFECTIVENESS_RATE
    + new_evidence_events(t) × EVIDENCE_SCRUTINY_IMPACT
```

| Parameter | Meaning | Default/Range |
|---|---|---|
| `active_scrutiny_score(t)` | Cumulative investigator attention on the operation | [0.0, 1.0]; 1.0 = maximum attention (charging decision imminent) |
| `SCRUTINY_NATURAL_DECAY` | Per-tick decay in investigator attention absent active evidence | 0.001 (decays ~3.6% annually without new leads) |
| `opsec_reduction(profile)` | Scrutiny reduction from current OPSEC profile (see table above) | [0.0, 0.75] |
| `OPSEC_EFFECTIVENESS_RATE` | Multiplier on OPSEC reduction (config-backed) | 1.0 (baseline; can be tuned per jurisdiction or NPC investigator skill) |
| `new_evidence_events(t)` | Count of new evidence tokens generated against this operation this tick | Integer; includes financial, testimonial, physical, documentary evidence |
| `EVIDENCE_SCRUTINY_IMPACT` | Scrutiny increase per new evidence event | 0.10 (each new evidence token adds 10% scrutiny) |

The OPSEC system provides a meaningful tradeoff: higher OPSEC profiles reduce investigator pressure but impose ongoing operational costs and delays. A player or criminal NPC choosing paranoid OPSEC loses 35–50% of revenue to overhead, slowing expansion, while a minimal-OPSEC operation moves fast and produces maximum profit until it's detected. The scrutiny formula ensures that OPSEC alone cannot indefinitely suppress a building investigation — new evidence always pushes scrutiny up, and OPSEC can only mitigate, not eliminate, the pressure.
...
**Regional Conditions Update Formulas:**

```
infrastructure(t+1) = infrastructure(t) × (1 - INFRASTRUCTURE_DECAY_RATE) + investment(t) / INFRASTRUCTURE_COST_UNIT
```

| Parameter | Meaning | Default/Range |
|---|---|---|
| `infrastructure(t)` | Cumulative regional infrastructure quality | [0.0, 1.0]; affects production efficiency, transportation speed, power stability |
| `INFRASTRUCTURE_DECAY_RATE` | Per-tick natural deterioration rate | 0.0002 (infrastructure loses ~7% annually at 0% investment) |
| `investment(t)` | Player and NPC capital invested in regional infrastructure this tick | Currency units; typically 0 or significant lump amounts |
| `INFRASTRUCTURE_COST_UNIT` | Currency value per infrastructure point | Config-backed; ~100,000 currency units per 0.01 infrastructure |

```
inequality_index(t) = gini(income_distribution(region))
```

Gini coefficient using standard formula: `gini = (Σ(2i - n - 1) × income_sorted[i]) / (n × Σ(income_sorted[i]))` where incomes are sorted ascending. Range [0.0, 1.0]: 0 = perfect equality, 1 = one person has all wealth. Computed each tick from aggregated NPC income distribution.

```
stability_score(t+1) =
    stability_score(t) + STABILITY_RECOVERY_RATE × (1 - stability_score(t))
    - instability_events(t) × EVENT_STABILITY_IMPACT
```

| Parameter | Meaning | Default/Range |
|---|---|---|
| `stability_score(t)` | Regional political/social stability | [0.0, 1.0]; affects NPC risk tolerance, investment decisions, crime rates |
| `STABILITY_RECOVERY_RATE` | Per-tick recovery toward 1.0 absent negative events | 0.001 (recovers ~3.6% per year) |
| `instability_events(t)` | Count of event types: violent crime surge, political assassination, protest/riot, labor conflict, natural disaster | Integer; each event type weighted equally |
| `EVENT_STABILITY_IMPACT` | Stability loss per event | −0.05 (each event reduces stability by 5%) |

```
criminal_dominance_index(t) = criminal_revenue(region, t) / total_economic_activity(region, t)
```

| Parameter | Meaning | Notes |
|---|---|---|
| `criminal_revenue(region, t)` | Sum of all criminal economy output (drug sales, protection rackets, trafficking, money laundering) measured in currency units | From criminal_sector = true transactions |
| `total_economic_activity(region, t)` | Sum of all legitimate + criminal economic activity | Denominator prevents trivial dominance in low-activity regions |
| Range: [0.0, 1.0] | Dominance escalates consequences at threshold 0.7 (§10.5) | |

```
addiction_rate(t+1) =
    addiction_rate(t) + new_addicts(t)/population(t)
    - recovery_rate × addiction_rate(t)
    - mortality_rate × addiction_rate(t)
```

| Parameter | Meaning | Default/Range |
|---|---|---|
| `addiction_rate(t)` | Fraction of working-age population in active addiction state | [0.0, 1.0]; sourced from individual NPC addiction_progression states |
| `new_addicts(t)` | Count of NPCs transitioning to active addiction this tick | Population-driven; increases with regional drug supply and decreases with treatment infrastructure |
| `population(t)` | Regional working-age population | From cohort simulation |
| `recovery_rate` | Fraction of addicted NPCs achieving recovery per tick | 0.001 (0.1% per tick; ~3.6% annually); increased by +0.005 per treatment facility operating in region |
| `mortality_rate` | Fraction of addicted NPCs dying from addiction-related causes per tick | 0.0005 (0.05% per tick; ~1.8% annually); increases with untreated addiction, decreases with healthcare investment |

All constants are configuration-backed in `simulation_config.json → regional_conditions`.

21. Update trust balances for all significant NPC relationships based on recent player actions
22. Update Influence network weights — apply obligation trust erosion for overdue open obligations (−`trust_erosion_rate` per tick, config-backed); clear stale movement_ally flags. Note: fear relationship decay does NOT run here — it runs via the DeferredWorkQueue as a batched operation every 30 ticks per NPC (see TDD §3). Trust-based rebuild from positive interactions is handled in step 21.
...
26a. Update technology progression — process R&D unlock completions, check era transition conditions, generate technology events (TDD §20)
27. Flush DeferredWorkQueue entries due this tick — process all WorkItems with `due_tick ≤ current_tick` (includes fear decay batches, evidence actionability decay, and all other deferred consequence items)
28. Update world map state; propagate cross-regional effects
...
**Consequence Delay Scheduling Formula:**

```
delay_ticks(consequence) =
    BASE_DELAY[consequence_type]
    × (1.0 + random_variance(seed, -0.2, 0.2))
    × awareness_modifier(source_npc)
```

| Parameter | Meaning | Range |
|---|---|---|
| `BASE_DELAY[consequence_type]` | Base delay in ticks by consequence category (see table below) | Ticks; typically 30–180 |
| `random_variance(seed, -0.2, 0.2)` | Pseudorandom multiplier per NPC/seed to avoid synchronized event clustering | [−0.2, 0.2]; added to 1.0 = multiplier [0.8, 1.2] |
| `awareness_modifier(source_npc)` | Adjustment based on source NPC's visibility/capability | [0.5, 2.0]; higher for NPCs with more resources/intelligence capability |

**Base Delay Table (by consequence type):**

| Consequence Type | Base Delay (ticks) | Notes |
|---|---|---|
| `financial_investigation` | 90 | Regulatory/LE review of financial records; ~3 months |
| `criminal_investigation` | 120 | Active criminal investigation building case; ~4 months |
| `media_exposure` | 45 | Journalist story publication; ~1.5 months |
| `political_consequence` | 60 | Political pressure, electoral response; ~2 months |
| `social_consequence` | 30 | Community reputation shifts, local response; ~1 month |
| `legal_proceeding` | 180 | Full legal process from charges to trial; ~6 months |
| `whistle_blower_contact` | 75 | Fired/disaffected employee contacts authority; ~2.5 months |
| `rival_escalation` | 45 | Criminal rival territorial response; ~1.5 months |

The awareness modifier accounts for the source NPC's resources and visibility. A well-connected regulator with internal LE contacts applies modifier 1.5–2.0 (acceleration), while a worker with minimal outside connections applies 0.5–0.8 (delay). A high-profile criminal organization applies 2.0 (immediate execution of planned retaliation), while a street-level operative applies 0.5 (slow, cautious response).
...
**EconLife — Commodities, Resources & Factory Specification** — complete goods list, supply chain tiers, factory recipes, quality system, and moddable data architecture.

**EconLife — Research, Development & Technology Progression** — R&D system, technology tree, climate model, era progression.

**EconLife — World Map & Geography Specification** — LOD simulation architecture, real-world data sources, province/nation hierarchy, scenario file system.

**EconLife — Procedural World Generation** — V1 procedural world generation pipeline (11 stages: tectonics through world commentary); H3 hexagonal grid spatial architecture; `WorldGenParameters` configuration; alternative to the GIS real-world pipeline.

All companion documents are living documents maintained in sync with this GDD.
...
- v1.1 — Pass3 Session 3: Section 2 setting block replaced with placeholder (Session 5 will write Year 2000 prose); cryptocurrency/CBDC paragraph removed from Section 2 (Year 2000 precondition — emergence is R&D unlock); "region/regions" → "province/provinces" throughout Section 2 Map subsection; January 2000 political boundary rule added to The Map; year-setting conflict with companion documents resolved
- v1.2 — Pass3 Session 5: Section 2 "The Setting" placeholder replaced with Year 2000 prose (400–700 word spec); arc framing paragraph added (2040s conditions are emergent outcomes, not starting conditions); companion document refs updated to TDD v5, FTL current; Section 23 companion list updated
- v1.3 — Pass3 Session 6: Full TDD cross-reference resolution pass; 18 sections updated to match TDD v16 authority; tick sequence expanded to 28+2 sub-steps; companion doc version numbers moved to document headers (§23 list now version-free)
- v1.4 — Economy consolidation pass: §8.8 informal economy reframed as legality/visibility overlay on unified simulation (not parallel layer); §12.1 architectural note added — criminal is a behavioral mode with `criminal_sector` + `visibility_scope` overlays, not a separate system; §12.6 Operational Security expanded to establish OPSEC as universal (all businesses), with criminal path as the `concealed` visibility_scope application; tick step 12 updated to reflect universal facility signals (TDD §16 FacilitySignals); companion TDD updated to v20
- v1.5 — Blocker fix pass: §6.8 whistleblower sign convention corrected throughout (emotional_weight < −0.6, not > 0.6); whistleblower condition updated to three-type set {witnessed_illegal_activity, witnessed_safety_violation, witnessed_wage_theft} per TDD §4 Session 14C; scenario tags updated to match; companion TDD updated to v21
- v1.6 — WorldGen scope promotion: §23 V1 Vertical Slice updated to include procedural world generation (11-stage WorldGenPipeline is V1, not EX); §23 Companion Documents updated to include Procedural World Generation companion doc; companion TDD updated to v22 (H3Index, LinkType, ProvinceLink, WorldGenParameters added to §12; Province struct updated with h3_index field and ProvinceLink adjacency)
- v1.7 — V1 completeness pass: weapons trafficking promoted to V1 in §23 exclusions; §10.5 consequence threshold named constants table added (P-05 resolution); §16 social media dynamics + media outlet ownership confirmed V1 scope throughout; GDD header session notes updated; companion TDD updated to v23

---

*EconLife GDD — v1.7 — Living Document*

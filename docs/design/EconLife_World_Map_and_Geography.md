...
*Companion document to GDD v1.7, Technical Design Document v29, R&D & Technology v2.3, Commodities & Factories v2.3*
*Version 1.2 — Pass 5 Session 8: Climate stress baseline, resource unlock era timing, GlobalCommodityPriceIndex LOD weighting, scenario override boundaries, all TDD references updated to v29*
...
#### Resource Unlock Era Timing [CORE] (Pass 5 Session 8)

**Critical clarification:** Technology nodes can unlock resources BEFORE their `era_available` date. `era_available` is a SOFT gate, not a hard gate:

- **Soft gate (era_available):** Resources become commercially viable at this era via normal progression. This is the historical baseline.
- **Technology hard gate:** Technology can override era gating. If `technology_unlocked(resource.required_tech_node)` is true, the resource is accessible regardless of `era_available`.

**Example:** Shale oil technology is a researchable node in Era 1 (can be researched early by player). WorldGen sets `shale_oil.era_available = 2`. However, once the player researches the shale_oil technology node, shale deposits unlock immediately, even in Era 1. The `era_available = 2` gate only applies to NPCs who haven't researched the technology yet.

This allows first-mover advantage without violating historical baselines.
...
4. Seeds `ClimateProfile.climate_stress_current` from the baseline at world load; the climate system modifies it forward from the starting CO₂ baseline

#### Climate Stress Baseline [CORE] (Pass 5 Session 8)

Game start baseline is set to January 2000 conditions:
- **climate_stress_index = 0.6** (corresponding to ~0.6°C above pre-industrial levels, matching year 2000 observational data)
- **CO2 index = 1.0** is a NORMALIZED value where 1.0 = year 2000 level (~370 ppm)
- **Temperature delta = climate_stress_index × TEMP_SENSITIVITY** (default: 3.0°C per doubling of CO2)

This baseline ensures the simulation starts with historical accuracy and compounds forward based on player economic choices. If a player builds entirely clean industries, climate stress decelerates relative to historical baseline. If a player builds fossil fuel-heavy economy, climate stress accelerates beyond historical path.
...
### Scenario Override Boundaries [CORE] (Pass 5 Session 8)

Scenario files may override these fields:
- `seed`
- `starting_era`
- `starting_year`
- `resource_abundance_multiplier`
- `climate_stress_initial`
- `technology_start_nodes[]` (whitelist of nodes pre-researched at game start)
- `nation_count`
- `province_count`

**Protected fields (NOT overridable by scenario):**
- H3 resolution (affects geographic fidelity; cannot change at runtime)
- `tick_duration` (core simulation timestep; breaks synchronization if changed mid-run)
- `LOD_thresholds` (LOD 0/1/2 boundaries; affects rendering and NPC simulation)
- `core_data_schema` (fundamental WorldState structure)

**Default behavior:** Any field not in the override whitelist is PROTECTED by default. Scenario files attempting to override protected fields are logged as warnings and the override is ignored. This prevents accidental simulation breaks via scenario modding.
...
### 7. Specify LOD 1 simplified tick — REPLACED by detailed sections below

The LOD 1 simplified tick has been fully specified in new sections. See "LOD 1 Monthly Tick Specification [CORE] [V1]" and "LOD 2 Annual Batch Job Specification [CORE] [V1]" below.

---

## Part 9 — LOD 1 Monthly Tick Specification [CORE] [V1]

**Responsibility:** The LOD 1 system runs a simplified monthly tick for all nations at LOD 1. It aggregates trade signals from LOD 0 (the player's nation) and produces monthly updates to `NationalTradeOffer` structures that feed back into LOD 0 market pricing.

### Module Structure

```cpp
struct LOD1TickProcessor {
    // Input interfaces
    struct Inputs {
        LOD0TradeData      lod0_border_trades;      // All LOD 0 → LOD 1 trade offers, cleared last month
        LOD0ClimateSignal  lod0_climate_contribution;  // CO2 from LOD 0; aggregated from last 12 ticks
        std::vector<Commodity> recent_lod0_prices;  // Last tick's LOD 0 market prices
    };

    // Output interfaces
    struct Outputs {
        std::vector<NationalTradeOffer> monthly_trade_offers;  // Per LOD 1 nation
        std::vector<float>               nation_gdp_estimates;   // Monthly GDP tier (relative scale)
        std::vector<TradeBalance>        nation_trade_balances;  // Exports - Imports per commodity
        std::vector<float>               commodity_price_signals; // LOD 1 nation expected prices
        std::vector<float>               population_change_monthly; // Birth rate, immigration pressure
        std::vector<float>               stability_index;        // 0.0–1.0 per nation; affects NPC behavior
    };

    // Processing
    void tick_lod1_nations(uint32_t game_year, uint32_t game_month);
};
```

### Data Flow

**Per LOD 1 nation, every month:**

1. **Read archetypes:** Load `nation_archetype` profile (trading tendency, tech tier, political risk, inflation baseline). Archetypes are nation-level behavioral templates, not individual NPCs.

2. **Aggregate from LOD 0:** Accumulate trade data flowing from LOD 0 to this LOD 1 nation over the past month. For each commodity, calculate:
   - Total LOD 0 exports to this nation (by commodity type)
   - Total LOD 0 imports from this nation
   - Effective prices observed in LOD 0 markets

3. **Calculate national aggregates:**
   - `nation_gdp_monthly` = baseline GDP from world.json × era economic growth factor × stochastic variance
   - `trade_surplus` = (LOD 0 imports from nation) − (LOD 0 exports to nation)
   - `commodity_prices` = weighted average of [LOD 0 prices (weight: observed trade volume), LOD 1 nation baseline prices (weight: 1.0 − volume_confidence)]
   - `population_change_monthly` = nation archetype fertility rate + (immigration pressure from LOD 0 labor demand)
   - `stability_index` = baseline (0.7 default) + archetype_volatility + (climate_stress effect) − (trade_surplus_stability_bonus)

4. **Generate trade offers:** Using `nation_gdp`, `commodity_prices`, and `nation_archetype`, produce a `NationalTradeOffer` for this nation detailing:
   - Which commodities this nation wants to import (up to 5–6 commodity types)
   - Quantity offered (as fraction of national GDP)
   - Price willing to pay (archetype negotiation factor applied)
   - Which commodities this nation wants to export
   - Quantity available
   - Minimum acceptable price

5. **Emit outputs:** Store generated outputs in `Outputs` struct.

**Performance budget:** < 50ms per LOD 1 nation per monthly tick (total tick run on ~10–20 LOD 1 nations: <1000ms).

### Aggregation Rules

- **NPC archetypes, not individuals:** LOD 1 nations do not simulate individual NPCs. All behavior derives from a single `NationArchetype` struct that encodes:
  - `inflation_baseline` — how stable this nation's currency is
  - `trade_openness` — 0.0 (protectionist) to 1.0 (free trader)
  - `political_volatility` — baseline stability for political events
  - `tech_tier_baseline` — what tech tier this nation operates at
  - `commodity_preferences` — which goods this nation specializes in
  - `population_growth_rate` — per-year fertility/immigration tendency

- **Facility archetypes, not instances:** LOD 1 nations do not track individual factories. Instead, the nation's production capacity is derived from:
  - `arable_land_fraction` × `population` → agricultural output capacity
  - `manufacturing_specialization` (archetype field) × `gdp` → factory output capacity
  - These update annually on milestones, not monthly

### Interface with LOD 0

**LOD 0 data flowing to LOD 1:**
- Trade offers from LOD 0 provinces to LOD 1 nations (aggregated per nation per commodity)
- LOD 0 commodity prices observed on markets
- LOD 0 CO₂ emissions (added to global climate stress)
- LOD 0 technological advances (propagate into LOD 1 nation tech tier on milestones)

**LOD 1 data flowing back to LOD 0:**
- `NationalTradeOffer` structures (LOD 0 provinces see these as availability to import from this nation)
- `commodity_price_signals` (feed into LOD 0 price equilibrium calculations)
- `stability_index` (affects diplomatic relationships, trade reliability)

---

## Part 10 — LOD 2 Annual Batch Job Specification [CORE] [V1]

**Responsibility:** The LOD 2 system does not tick monthly. Instead, once per 360 ticks (1 game year), a single batch job aggregates all LOD 2 nations' contributions to global systems.

### Schedule

- Runs on tick `N` where `N mod 360 == 0`
- Completes synchronously before any LOD 0 or LOD 1 tick in tick `N+1`
- Total runtime budget: < 200ms for all LOD 2 nations combined

### Inputs

```cpp
struct LOD2AnnualBatchJob {
    // Read from world.json / scenario
    struct LOD2NationProfile {
        std::string         nation_name;
        float               gdp_baseline;              // Year 2000 baseline from World Bank
        float               population;                // Year 2000 baseline from World Bank
        std::vector<float>  commodity_production;     // Baseline production per commodity type
        std::vector<float>  commodity_consumption;    // Baseline consumption per commodity type
        float               co2_emissions_gtons_yr;   // Year 2000 baseline from Our World in Data
        float               tech_tier_baseline;       // Era-adjusted technology tier
    };

    std::vector<LOD2NationProfile> lod2_nations;    // All LOD 2 nations in world
    GlobalCommodityPriceIndex      previous_index;  // Last year's global price index
    float                          global_climate_stress;  // Accumulated from all LOD levels
};
```
...
```cpp
struct LOD2AnnualBatchJob::Outputs {
    GlobalCommodityPriceIndex  updated_price_index;     // New global commodity prices
    float                      global_emissions_delta;  // Annual CO2 contribution to climate
    std::vector<TechnologyDiffusionEvent> tech_diffusion_events;  // New techs unlocked globally
};
```

### Processing

**For each commodity type (across all LOD 2 nations):**

1. **Aggregate production and consumption:**
   - `total_production_lod2` = sum of `commodity_production[commodity_id]` across all LOD 2 nations
   - `total_consumption_lod2` = sum of `commodity_consumption[commodity_id]` across all LOD 2 nations
   - `net_lod2_supply` = `total_production_lod2` − `total_consumption_lod2`

2. **Calculate LOD 2 price contribution:**
   - LOD 2 nations do not have individual market prices; instead they contribute a statistical baseline
   - `lod2_price_baseline[commodity_id]` = weighted average of:
     - All LOD 2 nations' production cost (derived from archetype productivity)
     - All LOD 2 nations' GDP (richer nations can pay more for scarce goods)
   - Weight by: `(lod2_nation.gdp / total_lod2_gdp)`

3. **Merge into GlobalCommodityPriceIndex** (Pass 5 Session 8):
   - The index is a weighted average of three sources:
     ```cpp
     GlobalCommodityPriceIndex(good) =
         LOD0_WEIGHT × lod0_price(good)          // 0.50
       + LOD1_WEIGHT × lod1_price_estimate(good) // 0.30
       + LOD2_WEIGHT × lod2_baseline(good)       // 0.20
     ```
   - **Weighting rationale:**
     - LOD0_WEIGHT = 0.50: LOD 0 is the most accurate (full simulation, real markets)
     - LOD1_WEIGHT = 0.30: LOD 1 monthly aggregates provide secondary signal
     - LOD2_WEIGHT = 0.20: LOD 2 annual estimates provide long-term global baseline
   - Weights can be tuned per scenario; defaults shown are for balanced play.

4. **Climate contributions:**
   - `total_co2_lod2_annual` = sum of `co2_emissions_gtons_yr` across all LOD 2 nations (adjusted for era)
   - `global_emissions_delta` += `total_co2_lod2_annual`
   - This is added to `ClimateState.global_co2_accumulation` for all nations to read in next tick

5. **Technology diffusion:**
   - For each technology node in the global tech tree:
     - Check if `percentage_of_lod2_gdp` with this tech > some threshold (e.g., 30%)
     - If yes, and if this tech is not yet unlocked globally, emit a `TechnologyDiffusionEvent`
     - Diffusion events flag that a tech has "entered the public domain" — all nations/players gain knowledge
     - Rate is limited: maximum 2–3 diffusion events per year to avoid tech explosion

### Performance Budget

- Reading all LOD 2 nation profiles: < 20ms
- Commodity aggregation loop: < 80ms (1000+ commodities, ~160 LOD 2 nations, single-threaded)
- Price index merge: < 50ms
- Climate aggregation: < 20ms
- Technology diffusion check: < 30ms
- **Total: < 200ms**

For faster generation, commodity aggregation can be parallelized across commodity types.

### Data Structures Needed in TDD

The following must be added or modified in TDD v29+:

```cpp
struct GlobalCommodityPriceIndex {
    std::map<uint32_t, float> commodity_base_prices;    // Per commodity ID: price in base currency
    std::vector<float>        lod1_price_contributions; // Weight of LOD 1 prices in average
    std::vector<float>        lod2_price_contributions; // Weight of LOD 2 prices in average
    uint32_t                  last_update_tick;         // For debug; should match annual batch job tick

    float get_price(uint32_t commodity_id) const {
        auto it = commodity_base_prices.find(commodity_id);
        return it != commodity_base_prices.end() ? it->second : 0.0f;
    }
};

struct NationArchetype {  // For LOD 1 nations
    std::string name;
    float       inflation_baseline;     // 0.98–1.02 per year
    float       trade_openness;         // 0.0–1.0
    float       political_volatility;   // How often political events fire
    float       tech_tier_baseline;     // Affects goods this nation can produce
    float       population_growth_rate; // Per-year fertility + immigration baseline
    std::vector<float> commodity_specialization;  // Which commodities this nation produces
};

struct TechnologyDiffusionEvent {
    uint32_t    technology_node_id;
    uint32_t    tick_diffused;
    std::string reason;  // "lod2_adoption_threshold" or similar
};
```

---

## Part 11 — Moddability Levels
...
**v1.1 → v1.2 (Pass 5 Session 4)**
- Added Part 9 — LOD 1 Monthly Tick Specification [CORE] [V1]: Detailed module structure, data flow, performance budget, aggregation rules, and LOD 0 interface for monthly LOD 1 nation updates (FIXES RWA-B1)
- Added Part 10 — LOD 2 Annual Batch Job Specification [CORE] [V1]: Schedule, inputs, outputs, processing logic for annual LOD 2 batch job including GlobalCommodityPriceIndex formula, climate contributions, and technology diffusion (FIXES RWA-B2)
- Updated Part 8 item 7 to reference new detailed specifications
- Renumbered Parts 9–12 to Parts 11–14 to accommodate new sections
- Updated version number and companion document references

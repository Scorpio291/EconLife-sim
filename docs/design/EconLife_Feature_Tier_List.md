...
*Companion document to GDD v1.7*
*Living document — updated in sync with GDD*
Cross-document fixes applied: C1; Pass 5 Session 3 (FT-B1, COM-B1); Pass 5 Session 7 (FT-I1, FT-I2, FT-I3, FT-I4)
...
| System | Tier | Rationale |
|---|---|---|
| Province attributes (demographics, resources, infrastructure) | V1 | Foundation of every province-level simulation |
| Dynamic province conditions (stability, inequality, crime rate) | V1 | Required for World Response System |
| One nation, 6 provinces | V1 | Defined scope for V1 |
...
| Mechanical history generation (forward-simulated pre-game history) | EXTENSION (adopted 2026-06-16) | Architect decision: the starting world becomes the *true outcome* of running the orchestrator for decades/centuries from a minimal planet-wide founding seed (settlements/firms/nations/economies develop, crash, evolve), rather than a hand-seeded snapshot with backward-narrated history. Scope is the **whole planet at LOD 2** (statistical), with the player's chosen region materialized to LOD 0 at entry — i.e., it extends the existing LOD system across deep time. Beyond documented V1 (which forward-sims from *loaded* conditions; worldgen is a staged non-realtime pipeline). Plan: docs/design/EconLife_Mechanical_History_Generation_Plan.md. Full-detail-everywhere (LOD 0 globally) remains EX. |
...
| Technology maturation (per-actor quality ceiling; ongoing maturation investment) | V1 | Depends on: R&D, Patents and licensing. Per-actor `maturation_level` (0.0–1.0) sets output quality ceiling; era-gated. First-mover advantage mechanic — pioneer's cumulative investment is a persistent quality gap competitors must close through independent investment, licensing, or espionage. See TDD v19 §3.3 WorkType::maturation_project_advance; R&D doc §Part 3.5. |
| Commercialization decision (hold proprietary / commercialize open / licensed / regional) | V1 | Depends on: R&D, Patents and licensing. Actor-controlled choice whether to bring a researched technology to open market, restrict to licensees, limit to specific regions, or keep internal only. NPC decision governed by business profile; player command is immediate. See TDD v19 §3.3 WorkType::commercialize_technology; R&D doc §Part 3.5 CommercializationDecision enum. |
...
| Cannabis, meth, opioids, synthetics | V1 | Full drug type list ships in V1 |
| Cocaine, heroin | V1 | Requires coca_leaf and poppy as Tier 0 biological resources; both [V1] |
...
| Weapons trafficking | V1 | Supply chain mirrors drug economy; supplier NPCs + entry conditions already specified in GDD §12.1; LE escalation path identical to drug enforcement |
| Protection rackets | V1 | Accessible early criminal revenue; low system complexity |
| Counterfeiting (currency, pharmaceutical, document) | EX | Expansion; document fraud needed for alternative identities — can be a simple unlock in V1. <!-- C1 fix --> Alternative identity creation requires basic document fraud capability. For V1, document fraud is implemented as a minimal unlock — a scene card interaction with a contact who has forger access, producing an AlternativeIdentity record. The full counterfeiting economy (currency and pharmaceutical counterfeiting) is EX. **Clarification (Pass 5 Session 7):** Document fraud (forging IDs, creating false identities) is [V1] scene card interaction mechanic. Counterfeit currency production is [V1] but requires counterfeiting_operation facility (separate from document fraud). These are SEPARATE mechanics — document fraud does not require a facility. |
...
| Social media dynamics | V1 | Required for complete media system; viral spread and amplification networks are the primary exposure vector in the 2000s+ timeline |
| Media outlet ownership | V1 | Required for player media path and editorial influence mechanic; depends on media system |
...
| Criminal sovereign | V1 | Full criminal economy (including weapons trafficking) is now V1; criminal sovereign is its natural endgame; mirrors legitimate sovereign as the covert power apex |
...
| Scene Card System | V1 | Templated scene cards with parameter injection (max branching depth: 3 choices per card); card types: interaction, negotiation, confrontation, discovery, consequence; triggered by simulation events; ~200 base templates covering core career paths in V1; [EX] scope: procedural card generation, community-authored cards |
| Scene cards for NPC interactions | V1 | Visual texture without 3D world; depends on: Scene Card System |
| Atmospheric scene settings (boardroom, parking garage, etc.) | V1 | Depends on: Scene Card System; part of information delivery |
...
*EconLife Feature Tier List — Companion to GDD v1.7 — Living Document*

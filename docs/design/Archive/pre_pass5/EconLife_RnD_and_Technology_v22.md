...
*Companion document to GDD v1.7, Technical Design Document v23, and Commodities & Factories v2.3*
*Version 2.2 — Cross-doc audit fixes: RnDFacility signals migration (OPSECProfile → FacilitySignals); TechnologyNode.key_technology_node removed (field belongs on FacilityRecipe, not tech tree); OQ 12 closed; companion version references updated*
...
- Permafrost thaw opens new Arctic resources while destabilizing existing northern infrastructure
...
struct RnDFacility {
    ...
    FacilitySignals signals;       // canonical struct: TDD v23 §16.
...
13. **Permafrost thaw dual-gate ownership.** WorldGen §1500 specifies that Arctic resources unlock when `regional_climate_stress > threshold` AND the arctic drilling tech is unlocked. No document owns the formula that combines these two gates into a single `ResourceDeposit.accessible` flag. Needs a home (likely TDD resource access section or WorldGen §8.7 extension).
...
## Change Log — v2.2 (Pass 3 Session 18 — Cross-Doc Audit Fixes)
...
*Companion document to TDD v23, GDD v1.7, Commodities & Factories v2.3, WorldGen v0.17*

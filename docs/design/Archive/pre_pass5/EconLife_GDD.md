...
**Obligation escalation:** What people ask for in return escalates based on creditor motivation and the player's visible wealth — not randomly. An obligation node progresses through four status tiers: **open** (owed but not yet demanded) → **escalated** (demand has grown beyond the original value) → **critical** (demand has reached 3× original, consequence queued) → **hostile** (creditor takes unilateral action via their expected_value engine). Escalation rate per tick = creditor's dominant motivation weight × `escalation_rate_base` × (1 + wealth_factor), where wealth_factor scales with the player's `visible_net_worth` — wealthier players face steeper demand growth because creditors read capacity to pay. A politician accepts a campaign contribution, then asks for a hire, then for influence over a regulatory decision, then for something that crosses into criminal exposure. Each step seems incremental. The cumulative commitment is not. All escalation rate constants are config-backed (see `simulation_config.json → obligations`). Full algorithm in TDD §7.
...
> [PATCH: Pass 3 Session 24 — Obligation Legibility Framework]
>
> ### 10.4a Obligation Legibility Framework
>
> The Obligation system has two explicit design intents that appear to conflict: the causality pillar ("a consequence that traces back to the player's decisions is worth more than any authored event") and the intentional opacity directive (§22.1: "the obligation escalation path should not come with a warning — the warning would destroy the experience"). This section resolves the tension by drawing a precise line between what is readable, what is opaque, and what is retrospectively reconstructable.
>
> **The resolution principle:** The favour acceptance is always readable. The escalation rate is intentionally opaque. The consequence, when it arrives, must be causally reconstructable. These three rules applied together satisfy both pillars simultaneously: the player is blindsided by the escalation mechanism (opacity), but can trace the full chain back to a specific decision they made (causality). The experience is surprise without unfairness.
>
> **Readable vs. Opaque table:**
>
> | What the player CAN see | Availability |
> |---|---|
> | That an ObligationNode exists | Always — node appears in the network view the moment the favour is accepted |
> | Creditor identity (name and role) | Always |
> | The original favour (what was received and what was given in return) | Always |
> | Favour date received | Always |
> | Current node status (open / escalated / critical / hostile) | Always — communicated via node colour, not text label |
> | Escalation history (demands made, dates, what was asked) | On demand — timeline view accessible from node detail panel |
> | That a new demand has arrived | Always — demand delivered via scene card; card arrival is visible |
>
> | What the player CANNOT see | Rationale |
> |---|---|
> | The `current_demand` float (exact numeric value) | Never shown. Demand is communicated through scene card dialogue in natural language, not as a tracked number. |
> | The escalation rate formula or per-tick growth | Never shown or explained. The first-time overlay is explicitly withheld for ObligationNode status transitions (see §22.1). |
> | When the next demand will arrive (deadline) | Never shown. No countdown indicator exists for escalation transitions. |
> | How the player's own wealth is affecting escalation speed | Never shown. The wealth_factor input to the escalation algorithm is internal only. |
>
> **The retrospective reconstructability requirement:** When a creditor transitions to `hostile` and takes unilateral action, the following must be true without exception:
> 1. The hostile action is linked to its originating ObligationNode — opening the node detail shows that this action came from this obligation.
> 2. The ObligationNode detail panel shows a complete escalation timeline: the original favour → each demand made → the hostile action taken and when.
> 3. The original favour entry includes what the player received, what they owed in return, and the date. The player can reconstruct the full chain. The system was not arbitrary — it was always traceable. They simply did not know they were being traced.
>
> **Obligation network view UI specification:**
>
> - **Node graph:** Each ObligationNode renders as a named node (creditor name and role label only; no status text). Edge colour communicates status: grey = `open`; amber = `escalated`; red = `critical`; black = `hostile`.
> - **Node detail panel (on click):** Favour description | Date received | Escalation history timeline (chronological list of demands and player responses) | Current status | Resolution options (active only when status is `escalated`, `critical`, or `hostile`).
> - **What is never shown in the UI:** The `current_demand` float. The escalation rate. Any numeric projection of future demands. Demand value is communicated only through the language of the scene card that delivers it.
> - **Alert badge (HUD):** Shows a count of nodes at `escalated` or higher. Badge is absent when all nodes are `open`. Amber badge for escalated-only; red badge if any node is `critical` or `hostile`. The badge is a signal that something is demanding attention — not a measure of how much is owed.
>
> **The first-time overlay decision (explicit):** The first time an ObligationNode is created, the first-time overlay does NOT fire. The contextual reveal system skips ObligationNode creation specifically. The player sees the node appear in their network view without explanation. The obligation exists. What it becomes is for them to discover.
>
> > [SCENARIO: scenario_player_reconstructs_obligation_chain]
> > Test: A player who receives a `hostile` action from an ObligationNode creditor can reconstruct the full causal chain from the obligation network view — from the original favour to the hostile action — without any information outside the network panel.
> > Seed setup: Player has one ObligationNode at status=`hostile` (creditor has taken a `report_to_law_enforcement` action); the node was created at tick 10 via a favour acceptance; it escalated to `escalated` at tick 200 and `critical` at tick 450; the hostile action was triggered at tick 520.
> > Run length: Inspect at tick 521.
> > Assertion: ObligationNode.detail_panel.escalation_history contains entries for: tick 10 (favour accepted), tick 200 (first escalation demand), tick 450 (critical escalation demand), tick 520 (hostile action with type and description). All entries are present in the node detail panel without requiring any additional data source.
...
**`street_reputation`** is a separate float on `PlayerCharacter.reputation.street` (internal range −1.0 to 1.0; display value = internal × 100, so the entry condition thresholds above are display values). Character creation starts at 0.0 (display: 0); maximum criminal reputation = 1.0 (display: 100). Negative values (display: negative) represent players known to be hostile to criminal networks — LE informants, anti-crime campaigners. It is built through: making criminal contacts, completing favors for criminal NPCs, being known within criminal networks (word propagates through `criminal_operator` relationship graphs). It is damaged by: cooperating with law enforcement, high-profile legitimate public activities that raise public profile without equivalent street activity. `street_reputation` is independent of `public_business`, `public_political`, and `public_social` — a player can have high public reputation and low street reputation simultaneously, or vice versa. The same mechanics apply to NPC criminal_operator reputation — the player is not a special case.
...
> [PATCH: Pass 3 Session 24 — Screen-Level UI Design]
>
> ### 20.6 Layer Navigation Model
>
> **Default view:** The Communications layer is foregrounded on game load, every session. The player's inbox is the entry point into the simulation — it is where the world talks to them. The Map and Operational layers are accessible from the moment the game starts but carry no data in session 1 until the player establishes a presence.
>
> **Layer switcher:** A persistent left-rail icon bar with three icons: Map, Operational, Communications. The active layer occupies the main viewport. No layer is hidden or locked from navigation (the Operational layer shows an empty state with guidance text when the player has no operations yet, rather than being greyed out). The Communications icon shows a badge count for unread cards (all classes combined).
>
> **Split view:** Any layer can be pinned as a side panel occupying up to 50% of the viewport. The active layer takes the remaining space. The side panel is persistent across interactions and layer switches. Recommended default pinned combinations (surfaced in a first-time overlay): Map + Communications (geographic context while reading incoming messages); Operational + Map (monitoring facilities while watching regional conditions). The player can set any combination or dismiss the side panel entirely.
>
> **Focus rules on event arrival:**
>
> | Event class | Layer focus behaviour |
> |---|---|
> | Mandatory scene card | Card appears as a full-viewport overlay over the active layer; no layer switch occurs; layer is dimmed and non-interactive until card resolves |
> | Timed-Optional card arrives | Card preview appears at bottom of active layer (non-obstructive); layer remains interactive; player can continue working or engage the card |
> | Ambient card | Badge increments on Communications icon; no focus change; active layer unaffected |
> | Map event (active event in coverage area) | If Map layer is active or pinned, event marker pulses; no forced focus change |
>
> ### 20.7 HUD Permanents
>
> The following elements are always visible regardless of the active layer. They constitute the minimum persistent information available to the player at all times.
>
> **Always-visible HUD elements:**
>
> | Element | Position | Format | Notes |
> |---|---|---|---|
> | In-game date and time | Top-left | "Tuesday, 14 March 2000" | Day of week + full date; no time of day display in V1 |
> | Character health badge | Top-right | Status badge: Healthy / Strained / Declining / Critical | Not a bar or a float — four named states only; colour-coded (green / amber / orange / red) |
> | Obligation alert badge | Top-right (adjacent to health) | Integer count of `escalated` + `critical` + `hostile` nodes | Absent when count is 0; amber when any node is `escalated`; red when any node is `critical` or `hostile`; does not display `current_demand` values |
> | Next calendar commitment | Bottom-left | Title of next committed entry + days until | "Board meeting — in 3 days"; blank when calendar is empty |
> | Fast-forward control | Bottom-right | Speed selector: 1× / 5× / 15× / 30× | Greyed and non-interactive when Mandatory card is pending |
>
> **Explicitly excluded from HUD:** Current cash balance (available in Operational layer, not HUD). Reputation scores (available in character profile, not HUD). Exposure level (never surfaced directly — see §10.2). Obligation `current_demand` floats (see §10.4a). Skill levels (character profile). The HUD communicates time, personal state, and urgent demands. All other information requires intentional navigation.
>
> ### 20.8 Information Density Levels
>
> Each layer has three zoom levels accessible via scroll or explicit zoom control. Zoom level determines information density — how much detail is presented for any given entity.
>
> **Map layer:**
> - *Overview* (region level): Province shapes colour-coded by stability index (green → red); facility count indicators per province; event activity density marker; contact coverage shading (no contacts = no shading; full contact network = solid fill).
> - *Operational* (facility level): Individual facility markers visible; NPC contact dots at their last known locations; active event pins; regional condition overlays available as toggleable layers (crime rate, inequality, infrastructure rating).
> - *Detail* (individual entity): Single facility or NPC profile panel; all available data for that entity rendered in full.
>
> **Operational layer:**
> - *Overview* (all operations): Summary tiles for each owned business, operation, or political position. Each tile shows: name, visual status state (healthy / degraded / critical, rendered visually before any metric), net cash flow indicator (arrow up/down/flat), and escalation badge if any.
> - *Operational* (single operation): Full dashboard for one business — department breakdown, production flow, staff status, financial reporting, delegation chain, escalation queue. Visual states applied before metrics per §20.4.
> - *Detail* (individual record): Single staff member, production line, or financial record. Full NPC or asset data.
>
> **Communications layer:**
> - *Overview* (inbox): All pending cards listed by priority class (Mandatory → Timed-Optional → Ambient), FIFO within class; card type icon, sender name, subject line; unread indicator.
> - *Operational* (card open): Single card at full presentation; dialogue choices or dismiss; NPC presentation state rendered per Scene Card Rulebook §5.
> - *Detail* (NPC or contact profile): Accessible from any scene card featuring an NPC; shows relationship history, known attributes, last interactions.
>
> ### 20.9 Scene Card Interrupt Behaviour (UI Specification)
>
> See Scene Card Rulebook (EconLife_Scene_Cards_v01.md) for full classification, queue ordering, and fast-forward rules. This section specifies only the visual interrupt behaviour in the UI context.
>
> **Mandatory card visual interrupt:** A full-viewport darkened overlay appears over the active layer. The scene card renders centred in the overlay at maximum readable size. The active layer is visible but dimmed and non-interactive. The layer switcher rail is non-interactive. HUD permanents remain visible. The scene card's painted illustration occupies the top two-thirds; dialogue and choice buttons occupy the bottom third.
>
> **Timed-Optional card preview:** A card preview panel slides up from the bottom of the active layer, occupying approximately 15% of viewport height. Preview shows: setting thumbnail, NPC name and role, first line of dialogue, countdown timer. Tapping the preview brings the card to full viewport display. Dismissing from preview fires the default outcome immediately. The active layer remains interactive above the preview panel.
>
> **Ambient card:** No visual interrupt. Badge increments. No preview panel.
>
> **Scene card container design:** The painted scene illustration is the only visual element in the interface with a rough, painted character. The card container (border, dialogue area, choice buttons) is clean typographic UI consistent with the rest of the interface. The contrast between the painted illustration and the typographic UI chrome is intentional and must be maintained — do not apply painterly or textured styling to UI elements, and do not apply clean digital rendering to scene card illustrations.
>
> ### 20.10 Obligation Network View
>
> The obligation network view is accessible from the HUD obligation alert badge (tap/click) or from the Operational layer character panel. It is a dedicated panel, not a layer — it overlays the current view and can be dismissed at any time.
>
> **Panel contents:**
> - *Node graph:* Each ObligationNode renders as a named node with creditor name and role. Node edge colour communicates status: grey (`open`), amber (`escalated`), red (`critical`), black (`hostile`). No numeric values are displayed on the graph.
> - *Node detail (on click):* Favour description | Date received | Escalation history timeline (chronological list: favour accepted → each demand made → current status) | Resolution options (active when status is `escalated`, `critical`, or `hostile`; greyed for `open` nodes). The `current_demand` float is not displayed anywhere in the panel.
> - *Resolution options panel:* When status is `escalated` or higher: Pay it / Renegotiate / Eliminate / Preemptive exposure. Each option shows a brief consequence description. The panel does not show probability estimates — outcomes are not quantified.
>
> See §10.4a for the full readable/opaque specification governing what information this view displays.
>
> ### 20.11 First-Session Layer State
>
> The following states apply at game start (session 1, immediately after character creation):
>
> | Layer / Element | State | Unlock Condition |
> |---|---|---|
> | Communications layer | Active, foregrounded, default | Always available |
> | Map layer | Active but empty (no contact network = no data) | Always accessible; populates as contacts are established |
> | Operational layer | Active but shows empty state ("No operations yet") | Always accessible; populates on first business/operation acquisition |
> | Split view | Available | Always |
> | Obligation alert badge | Present, dormant (count = 0) | Badge activates on first ObligationNode creation |
> | Obligation network panel | Accessible but empty | Populates on first ObligationNode creation; no first-time overlay fires at this moment |
> | Fast-forward control | Available at all speeds | Always |
>
> The Operational layer is never locked or hidden — showing an empty state preserves discoverability. The player can navigate to it immediately and understand from context what it will contain when they have operations.

# EconLife — Technical Design Document v17 — Pass 3 Session 17 Complete
*Companion documents: GDD v1.3, Feature Tier List, Commodities & Factories v2.1, R&D & Technology v1.1, World Map & Geography v1.1*
*Living document — updated in sync with GDD and Feature Tier List*
*Pass 2 complete; Pass 3 Sessions 1–4 complete; Pass 3 Session 6 (Moddability) complete; Pass 3 Session 7 (Architecture: Packages, Parallelism, Transit, Persistence) complete; Pass 3 Session 8 (Physics Consistency Pass) complete; Pass 3 Session 9 (Structural Fixes) complete; Pass 3 Session 10 (Missing Struct Definitions) complete; Pass 3 Session 11 (Precision Gap Completions) complete; Pass 3 Session 12 (Mechanical Fixes) complete; Pass 3 Session 13 (Universal Facility Signals and Scrutiny System) complete; Pass 3 Session 14 (Grey Zone Compliance and Universal Community Grievance) complete; Pass 3 Session 15 (Player-Special Fixes) complete; Pass 3 Session 16 (Constants Homing) complete — see Pass3_Index.md for current status*
...
enum class WorkType : uint8_t {
    ...
    risk_drift,                 // periodic baseline drift for one NPC's risk_tolerance (§4.6)
};
...
struct NPC {
    ...
    float risk_tolerance;                 // 0.0–1.0; current value; modified by life events (§4.6)
    float baseline_risk_tolerance;        // 0.0–1.0; value at NPC creation; drift target (§4.6)
...
### 4.6 — Risk Tolerance Change Mechanism [V1]

`risk_tolerance` is not static after NPC creation. Life events modify it through the memory system. Two sub-mechanisms: an event-triggered delta applied when qualifying memory entries are written, and a periodic drift back toward the NPC's `baseline_risk_tolerance`.

#### Risk-modifying MemoryType values

New MemoryType values added to the existing open set:

| Memory type | Base delta | Description |
|---|---|---|
| `victimization` | −0.08 | Robbed, assaulted, arrested as victim, imprisoned |
| `competence_gain` | +0.06 | Martial arts, security training, military service, professional certification of threat-management skill |
| `criminal_burn` | −0.10 | Betrayed, set up, near-arrested in a criminal context |
| `dependents_gained` | −0.05 | New child, partner with dependents, aging parent now reliant on NPC |
| `dependents_lost` | +0.04 | Death or departure of a dependent — grief variant (see desperation exception below) |
| `desperation` | +0.07 | Prolonged scarcity; survival motivation threshold crossed; nothing left to protect |
| `successful_risk` | +0.04 | Took a risk; outcome was positive (reinforcement learning) |
| `failed_risk` | −0.06 | Took a risk; outcome was negative (deterrence) |
| `peer_norm_shift` | +0.05 | Joined network where risk-taking is normalized and visibly rewarded |

Base delta values are config-backed (`simulation_config.json`, `risk_delta_table` block) — not hardcoded.

#### apply_risk_delta()

Called inside the memory-writing path whenever a qualifying MemoryEntry is appended to an NPC's `memory_log`. Cost is at write time, not at tick evaluation.

```cpp
void apply_risk_delta(NPC& npc, const MemoryEntry& entry, uint32_t current_tick):
    if entry.type not in RISK_MODIFYING_TYPES:
        return

    base_delta = config.risk_delta_table[entry.type]

    // Desperation exception: when an NPC is survival-dominant AND in prolonged scarcity,
    // victimization flips to risk-increasing. The expected cost of inaction has reached
    // the expected cost of action — the deterrent effect of a bad outcome collapses.
    if entry.type == victimization:
        if npc.motivations[survival] > config.desperation_survival_threshold
           AND npc_in_scarcity_state(npc, current_tick):
            base_delta = abs(base_delta)   // flip: victimization now drives risk UP

    // Motivation gating: stability-motivated NPCs amplify downward pressure and dampen
    // upward pressure. Power/revenge-motivated NPCs resist downward pressure.
    stability_pull = npc.motivations[stability] - npc.motivations[power]
    motivation_mod = 1.0f + stability_pull * config.risk_motivation_sensitivity
    if base_delta > 0:
        motivation_mod = 2.0f - motivation_mod   // invert for upward deltas

    // Scale by event emotional weight — larger events move the needle more
    delta = base_delta * abs(entry.emotional_weight) * motivation_mod

    npc.risk_tolerance = clamp(
        npc.risk_tolerance + delta,
        config.risk_tolerance_floor,     // 0.05 — never fully paralysed
        config.risk_tolerance_ceiling    // 0.95 — never fully reckless
    )
```

#### Baseline drift

Runs as a `WorkType::risk_drift` DeferredWorkItem. Each NPC gets one scheduled at creation; it self-reschedules after every execution.

```cpp
void drift_risk_toward_baseline(NPC& npc, uint32_t current_tick):
    gap = npc.baseline_risk_tolerance - npc.risk_tolerance
    if abs(gap) < config.risk_drift_floor:
        // within floor — no adjustment this cycle; still reschedule
        schedule_deferred(WorkType::risk_drift, npc.id,
                          current_tick + config.risk_drift_interval)
        return

    npc.risk_tolerance += gap * config.risk_drift_rate
    schedule_deferred(WorkType::risk_drift, npc.id,
                      current_tick + config.risk_drift_interval)
```

Drift closes a fraction of the gap per cycle — it never fully resets to baseline if events keep accumulating, because every new event appends a new delta before the drift closes the previous one. Long-term traumatised NPCs stay shifted.

#### Config constants (`simulation_config.json`)

```json
"risk_tolerance_floor":           0.05,
"risk_tolerance_ceiling":         0.95,
"risk_drift_rate":                0.02,
"risk_drift_interval":            30,
"risk_drift_floor":               0.01,
"risk_motivation_sensitivity":    0.30,
"desperation_survival_threshold": 0.70,
"risk_delta_table": {
    "victimization":     -0.08,
    "competence_gain":   +0.06,
    "criminal_burn":     -0.10,
    "dependents_gained": -0.05,
    "dependents_lost":   +0.04,
    "desperation":       +0.07,
    "successful_risk":   +0.04,
    "failed_risk":       -0.06,
    "peer_norm_shift":   +0.05
}
```

`risk_drift_interval = 30` matches the fear decay cadence — both use the same DeferredWorkQueue rhythm. This is intentional: they represent the same class of slow emotional normalization.

**Design note:** This mechanism is implicitly validated by `scenario_witness_delayed_cooperation_on_circumstance_change` (GDD §3), which asserts `risk_tolerance` rises above action threshold between years 1 and 4 for a witness NPC following circumstance improvement. The drift + delta model must produce that outcome from its parameters. No dedicated scenario tag is needed — the existing tag covers it.
...
```
`TECH_QUALITY_CEILING_BASE` and `TECH_QUALITY_CEILING_STEP` are also in `economy.json`. They are independent of the output/cost constants above — output bonus is about volume; quality ceiling is about the maximum achievable output quality grade.

**R&D document cross-reference:** `FacilityRecipe.tech_tier_output_bonus` (R&D doc) is a per-recipe override field. When present, that value replaces `TECH_TIER_OUTPUT_BONUS_PER_TIER` for that specific recipe. The global constant is the default; the recipe field applies where the standard rate is inappropriate.
...
- v17: §4.6 Risk Tolerance Change Mechanism added — baseline_risk_tolerance field, apply_risk_delta(), drift_risk_toward_baseline(), WorkType::risk_drift, 9 new MemoryType values with config-backed delta table; GDD companion updated to v1.3

---

*EconLife Technical Design Document v17 — Pass 3 Session 17 Complete — Companion to GDD v1.3*

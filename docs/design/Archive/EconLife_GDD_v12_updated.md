...
**`street_reputation`** is a separate float on `PlayerCharacter.reputation.street` (range −1.0 to 1.0; displayed in-game as a 0–100 index). It is built through: making criminal contacts, completing favors for criminal NPCs, being known within criminal networks (word propagates through `criminal_operator` relationship graphs). It is damaged by: cooperating with law enforcement, high-profile legitimate public activities that raise public profile without equivalent street activity. `street_reputation` is independent of `public_business`, `public_political`, and `public_social` — a player can have high public reputation and low street reputation simultaneously, or vice versa.
...
**EconLife — Technical Design Document v19** — simulation architecture specification, data structures, performance targets, prototype scope, and known technical risks.

**EconLife — Commodities, Resources & Factory Specification v2.3** — complete goods list, supply chain tiers, factory recipes, quality system, and moddable data architecture.

**EconLife — Research, Development & Technology Progression v2.2** — R&D system, technology tree, climate model, era progression, technology lifecycle model.

**EconLife — World Map & Geography Specification v1.1** — LOD simulation architecture, real-world data sources, province/nation hierarchy, scenario file system.
...
*EconLife GDD — v1.2 — Living Document*

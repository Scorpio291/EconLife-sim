...
- **Grievance is owned solely by this module** (it is the only writer of the baseline `grievance_level`; discrete event spikes still arrive via `RegionDelta.grievance_delta` from `media_system`, `protection_rackets`, and fired social consequences). The grievance sample is material-deprivation grounded plus a bounded actor-wrong (memory) component, and EMA-smoothed — the material grounding is the restoring force that a memory-only accumulator lacked: `grievance_sample = clamp(material + grievance_memory_weight * memory, 0, 1)` where `material = clamp(grievance_unemployment_weight * unemployment_rate + grievance_inequality_weight * inequality_index, 0, 1)` and `memory = min(grievance_sum / (npc_count * grievance_normalizer), 1.0)`. When the economy is healthy, grievance relaxes toward a low baseline instead of saturating.
- Grievance memory contribution per NPC: sum of `abs(entry.emotional_weight) * action_type_weight(entry.type)` for negative attribution memory entries where `decay > config.community.memory_decay_floor`. Action type weights: direct harm types (witnessed_illegal_activity, witnessed_safety_violation, witnessed_wage_theft, physical_hazard, retaliation_experienced) = 1.0; economic harm types (employment_negative, facility_quality) = 0.5; all others = 0.0.
- Escalation flows grievance → stage. The earlier stage → grievance bonus (a self-reinforcing pump) has been removed; stage intensity no longer feeds back into grievance.

...
- `NPCDelta.updated_relationship` -- trust erosion from overdue obligations, written as an upsert on the player's relationship with the creditor (keyed by `target_npc_id`). The struct carries **delta-shaped** values — `trust = obligation_balance = -0.001` per overdue tick — because apply_deltas merges trust/obligation_balance additively into an existing relationship (the struct is initial values only on insert; sending the eroded absolute value would double-count). `recovery_ceiling` is sent as 1.0 ("no change" under apply's ratchet-down merge), or as `max(0.60 × post-erosion trust, 0.15)` on hostile escalation.
...
- Overdue obligations erode trust: a `-0.001/tick` **delta** sent via `NPCDelta.updated_relationship` (apply merges additively into the player's relationship with the creditor, establishing it if absent).

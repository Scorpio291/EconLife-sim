...
- **Current schema version**: `CURRENT_SCHEMA_VERSION = 6` (defined in `persistence_module.h`).
...
- **Rejection policy**: pre-v6 saves are **rejected outright** by `is_schema_compatible(saved < 6) == false`. Each schema bump that changes record layout (not just appends a new top-level field) is a hard rejection: a v5 save loaded by v6 code would short-read at the new trailing blocks; a v4 RegionConditions has 4 trailing fields that v5 reads as part of the next struct; a v3 save loaded by v4 code would short-read every NPC; a v2 save loaded by v3 code would silently route every market delta to a phantom market. V1 is pre-release so this affects no real users.

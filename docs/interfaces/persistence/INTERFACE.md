...
- **Current schema version**: `CURRENT_SCHEMA_VERSION = 3` (defined in `persistence_module.h`).
- **Schema version history**:
  ...
  - **v3** — embedded `WorldState::goods_catalog` (numeric_id, good_id, display_name, tier, unit, category, base_price, perishable, illegal, era_available per entry) and changed the interpretation of `good_id` fields on `RegionalMarket`, `MarketDelta`, and recipe outputs from FNV-1a string hashes to catalog `numeric_id` assigned at CSV load. Pre-v3 saves are **rejected outright** by `is_schema_compatible(saved < 3) == false`: loading a v2 save with v3 code would silently route every market delta to a phantom market (the FNV hash of a string vs. the catalog's numeric_id are unrelated values). V1 is pre-release so this affects no real users.

...
- **Schema versioning**: `CURRENT_SCHEMA_VERSION` bumped on every migration-requiring change. Saves with `loaded_schema_version > CURRENT_SCHEMA_VERSION` rejected (`RestoreResult::schema_too_new`). Saves with older versions migrated in version order via `MigrationRegistry`, except where a breaking semantics change makes migration impossible — those saves are rejected outright by `is_schema_compatible`.
- **Migration contract**: migrations are additive only. Add new fields with defaults; never delete data. Breaking semantics changes (e.g. the v2 → v3 cutover on `good_id` interpretation) bump the version floor in `is_schema_compatible` rather than registering a migration.
- **Current schema version**: `CURRENT_SCHEMA_VERSION = 7` (defined in `persistence_module.h`).
- **Schema version history**:
  - **v1** — initial release format.
  - **v2** — added persistence of `WorldState::next_action_sequence` so the monotonic player-action sequence counter survives save/load. See `docs/design/EconLife_PlayerDelta_Semantics_v1.md` (issue #11).
  - **v3** — embedded `WorldState::goods_catalog` (numeric_id, good_id, display_name, tier, unit, category, base_price, perishable, illegal, era_available per entry) and changed the interpretation of `good_id` fields on `RegionalMarket`, `MarketDelta`, and recipe outputs from FNV-1a string hashes to catalog `numeric_id` assigned at CSV load.
  - **v4** — added `NPC::addiction_state` (stage, substance_key, tolerance, craving, consecutive_use_ticks, clean_ticks, supply_gap_ticks, relapse_probability) to each NPC record. Resolves the spec-vs-code drift where `addiction/INTERFACE.md` listed `NPC.addiction_state` as the field while the implementation kept state in a module-private map.
  - **v5** — demographics consolidation. Population-fraction monitors (`addiction_rate`, `crime_rate`, `criminal_dominance_index`, `formal_employment_rate`) migrated from `RegionConditions` to `RegionCohortStats`; new monitors (`sick_rate`, `homeless_rate`, `unemployment_rate`) added to `RegionCohortStats`. Per-province serialization: `RegionConditions` is 4 floats shorter; `RegionCohortStats` is 7 floats longer (when present).
  - **v6** — added trailing serialization blocks after the route table: `state.currencies` (vector of `CurrencyRecord` — nation_id, iso_code, usd_rate, usd_rate_baseline, volatility, foreign_reserves, pegged, peg_rate), `state.facilities` (vector of `Facility` — id, business_id, province_id, recipe_id, tech_tier, output_rate_modifier, soil_health, worker_count, is_operational), and a `GlobalTechnologyState` footer covering `current_era`, `era_started_tick`, `domain_knowledge[19]`, `active_research_projects`, and `active_maturation_projects`. `era_triggers` stays config-loaded reference data and is reloaded on startup — not persisted. Closes critical save/load data-loss gaps: exchange rates, production facilities, era progression, and in-progress research/maturation now survive round-trips.
  - **v7** — added a trailing module-private state section. Layout: `u32 count`, then per module a length-prefixed string `name` + `u32 size` + `size` payload bytes. Modules opt in by overriding `ITickModule::serialize_state` / `deserialize_state`. Each module's payload format is internal to that module — `persistence_module` only routes blocks by name. Two new `PersistenceModule::serialize` / `deserialize` overloads accept an optional `std::vector<ITickModule*>` for opt-in modules; the zero-argument overload still works and serializes a `count=0` section. Modules using this hook in V1 (rounds 1+2+3+4 cover nineteen modules; closes every flagged module-private-state data-loss gap with cross-tick run state): `random_events` (active_events_, next_event_id_), `protection_rackets` (rackets_), `real_estate` (properties_), `legal_process` (cases_), `money_laundering` (operations_, fiu_results_), `labor_market` (job_postings_, employment_records_, npc_skills_, regional_wages_, applications_), `investigator_engine` (cases_), `banking` (active_loans_, borrower_credits_), `criminal_operations` (organizations_ including nested member_npc_ids/income_source_ids/dominance_by_province map + active_expansions_), `informant_system` (records_), `drug_economy` (production_records_, legalization_status_), `healthcare` (province_health_states_, npc_health_records_), `community_response` (province_states_), `media_system` (outlets_ including journalist_ids + active_stories_ including evidence_token_ids), `government_budget` (budgets_ including spending_allocations and spending_actual std::maps), `designer_drug` (compounds_ including goods_key string), `antitrust` (proposals_), `commodity_trading` (positions_), `financial_distribution` (compensation_records_ including nested EquityGrant vector and BoardComposition member roster). Modules that hold unordered_map state (labor_market) canonicalise key order on write; std::map members (criminal_operations.dominance_by_province) iterate in key order already. Modules in the save but absent from the load `modules` list are skipped (forward compatibility for partial loads).
- **Rejection policy**: pre-v7 saves are **rejected outright** by `is_schema_compatible(saved < 7) == false`. Each schema bump changes the byte-stream layout in a way that older code would short-read or misalign; V1 is pre-release so this affects no real users. The chain: v3 catalog → v4 NPC addiction footer → v5 cohort_stats migration → v6 currency/facility/technology footers → v7 module-state section.

...
- **Current schema version**: `CURRENT_SCHEMA_VERSION = 4` (defined in `persistence_module.h`).
...
- **Rejection policy**: pre-v4 saves are **rejected outright** by `is_schema_compatible(saved < 4) == false`. Each schema bump that changes record layout (not just appends a new top-level field) is a hard rejection: a v3 save loaded by v4 code would short-read every NPC (missing the addiction footer); a v2 save loaded by v3 code would silently route every market delta to a phantom market. V1 is pre-release so this affects no real users.

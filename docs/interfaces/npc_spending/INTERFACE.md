...
- Background population demand: `demand_contribution(cohort, g) * cohort.population_size`, using cohort-mean disposable income and modal `BuyerType`. *(The general per-good cohort model remains aspirational; the divergence note below records what is actually implemented.)*
- **Implemented — background population food subsistence (Part C).** Independent of the significant-NPC pass, each province's background population (`cohort_stats->total_population`) generates inelastic necessity demand for a configured **food basket** (`NpcSpendingConfig::food_basket`): `food_need = total_population * per_capita_food_per_tick`, distributed across the basket goods present in the province **proportional to their available supply** (even split when the basket has no supply, so unmet demand still signals scarcity). This demand is **fused into the per-market total before the single capped consumption**, so background and NPC demand never double-spend the same supply: `consumed = min(total_demand, available)`, `demand_buffer_delta = total_demand` (full signal to `price_engine`), `supply_delta = -consumed`. The background population is **not charged cash** (cohorts are abstract — only significant NPCs get an `NPCDelta`); the demand raises food prices, which food producers earn against. The module now runs whenever a province has either significant NPCs **or** a non-zero `total_population` (previously it returned early with no significant NPCs). Consumed food contributes to `municipal_waste` via `civilian_waste_rate` like all consumption. Food scarcity has no wellbeing/health consequence yet — that feedback is the next increment.

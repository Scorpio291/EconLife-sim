...
- **Energy (electricity as a generated, consumed good).** Before facilities run, an energy pre-pass per province computes electricity demand (sum of `energy_per_tick` over operational, era-available facilities), generates it from the province endowment — renewables (`SolarPotential`/`WindPotential`/`Geothermal` deposit capacity + hydro from `river_flow_regime`) matter-free, the shortfall by **burning** the province's fossil-fuel stock (`crude_oil`/`natural_gas`/`thermal_coal`), consuming that matter (`supply_delta < 0`) — and emits the `electricity` good. The brownout ratio `clamp(generation/demand, 0, 1)` scales every facility's output that tick. Recipes with `energy_per_tick == 0` are unaffected (ratio 1.0). Energy is conserved: fuel matter → electricity → work; renewables are free flows.
